...
static void ossl_error(const char *msg) {
  ...
  while ((e = ERR_get_error()) != 0)
    error("%s: libcrypto error: %.100s", __func__, ERR_error_string(e, NULL));
}
#endif /* HAVE_EC_KEY_METHOD_NEW */

int pkcs11_init(int interactive) {
 ...
 */
static void pkcs11_provider_finalize(struct pkcs11_provider *p) {
  CK_RV rv;
  CK_ULONG i;

  debug("pkcs11_provider_finalize: %p refcount %d valid %d", p, p->refcount,
        p->valid);
  if (!p->valid)
    return;
  for (i = 0; i < p->nslots; i++) {
    if (p->slotinfo[i].session && (rv = p->function_list->C_CloseSession(
...
int pkcs11_del_provider(char *provider_id) {
  struct pkcs11_provider *p;
...
static void pkcs11_k11_free(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
...
static int pkcs11_find(struct pkcs11_provider *p, CK_ULONG slotidx,
                       CK_ATTRIBUTE *attr, CK_ULONG nattr,
  ...
  if ((rv = f->C_FindObjects(session, obj, 1, &nfound)) != CKR_OK ||
      nfound != 1) {
    debug("C_FindObjects failed (nfound %lu nattr %lu): %lu", nfound, nattr,
          rv);
  } else
    ret = 0;
...
static int pkcs11_login_slot(struct pkcs11_provider *provider,
                             struct pkcs11_slotinfo *si, CK_USER_TYPE type) {
  ...
  if (si->token.flags & CKF_PROTECTED_AUTHENTICATION_PATH)
    ...
    snprintf(prompt, sizeof(prompt), "Enter PIN for '%s': ", si->token.label);
...
static int pkcs11_login(struct pkcs11_key *k11, CK_USER_TYPE type) {
...
static int pkcs11_check_obj_bool_attrib(struct pkcs11_key *k11,
                                        CK_OBJECT_HANDLE obj,
  ...
  *val = flag != 0;
  debug("%s: provider %p slot %lu object %lu: attrib %lu = %d", __func__,
        k11->provider, k11->slotidx, obj, type, *val);
  return (0);
}

static int pkcs11_get_key(struct pkcs11_key *k11, CK_MECHANISM_TYPE mech_type) {
  struct pkcs11_slotinfo *si;
  CK_FUNCTION_LIST *f;
  CK_OBJECT_HANDLE obj;
  CK_RV rv;
...
static int pkcs11_rsa_private_decrypt(int flen, const u_char *from, u_char *to,
                                      RSA *rsa, int padding) {
  return (-1);
}

static int pkcs11_rsa_start_wrapper(void) {
  if (rsa_method != NULL)
    return (0);
  ...
  if (rsa_method == NULL)
    return (-1);
  rsa_idx =
      RSA_get_ex_new_index(0, "ssh-pkcs11-rsa", NULL, NULL, pkcs11_k11_free);
  if (rsa_idx == -1)
    return (-1);
...
static int pkcs11_ecdsa_start_wrapper(void) {
  ...
  if (ec_key_method != NULL)
    return (0);
  ec_key_idx = EC_KEY_get_ex_new_index(0, "ssh-pkcs11-ecdsa", NULL, NULL,
                                       pkcs11_k11_free);
  if (ec_key_idx == -1)
    return (-1);
...
static int pkcs11_ecdsa_wrap(struct pkcs11_provider *provider, CK_ULONG slotidx,
...
static void rmspace(u_char *buf, size_t len) {
  size_t i;
 ...
 */
static int pkcs11_open_session(struct pkcs11_provider *p, CK_ULONG slotidx,
                               char *pin, CK_ULONG user) {
  struct pkcs11_slotinfo *si;
  CK_FUNCTION_LIST *f;
  CK_RV rv;
  CK_SESSION_HANDLE session;
  ...
  if ((rv = f->C_OpenSession(p->slotlist[slotidx],
                             CKF_RW_SESSION | CKF_SERIAL_SESSION, NULL, NULL,
                             &session)) != CKR_OK) {
  ...
  if (login_required && pin != NULL && strlen(pin) != 0) {
    ...
    if (rv != CKR_OK && rv != CKR_USER_ALREADY_LOGGED_IN) {
      ...
      ret = (rv == CKR_PIN_LOCKED) ? -SSH_PKCS11_ERR_PIN_LOCKED
...
#ifdef HAVE_EC_KEY_METHOD_NEW
static struct sshkey *pkcs11_fetch_ecdsa_pubkey(struct pkcs11_provider *p,
                                                CK_ULONG slotidx,
                                                CK_OBJECT_HANDLE *obj) {
...
static struct sshkey *pkcs11_fetch_rsa_pubkey(struct pkcs11_provider *p,
                                              CK_ULONG slotidx,
                                              CK_OBJECT_HANDLE *obj) {
  CK_ATTRIBUTE key_attr[3];
  CK_SESSION_HANDLE session;
...
static int pkcs11_fetch_x509_pubkey(struct pkcs11_provider *p, CK_ULONG slotidx,
  ...
  *labelp = NULL;
  ...
  if ((x509_name = d2i_X509_NAME(NULL, &cp, cert_attr[1].ulValueLen)) == NULL ||
 ...
 */
static int pkcs11_fetch_certs(struct pkcs11_provider *p, CK_ULONG slotidx,
  ...
  while (1) {
    ...
    switch (ck_cert_type) {
    case CKC_X_509:
      if (pkcs11_fetch_x509_pubkey(p, slotidx, &obj, &key, &label) != 0) {
        error("failed to fetch key");
        continue;
      }
      break;
    default:
      error("skipping unsupported certificate type %lu", ck_cert_type);
      continue;
    }
...
pkcs11_rsa_generate_private_key(struct pkcs11_provider *p, CK_ULONG slotidx,
  ...
  *err = 0;
  ...
  FILL_ATTR(tpub, npub, CKA_VERIFY_RECOVER, &false_val, sizeof(false_val));
  ...
  FILL_ATTR(tpub, npub, CKA_PUBLIC_EXPONENT, pubExponent, sizeof(pubExponent));
  ...
  FILL_ATTR(tpriv, npriv, CKA_SIGN_RECOVER, &false_val, sizeof(false_val));
...
static int pkcs11_decode_hex(const char *hex, unsigned char **dest,
                             size_t *rlen) {
  size_t i, len;
  char ptr[3];
 ...
 */
static int pkcs11_register_provider(char *provider_id, char *pin,
                                    struct sshkey ***keyp, char ***labelsp,
                                    struct pkcs11_provider **providerp,
                                    CK_ULONG user) {
  ...
  *providerp = NULL;
  ...
  if (pkcs11_provider_lookup(provider_id) != NULL) {
    debug("%s: provider already registered: %s", __func__, provider_id);
    goto fail;
  }
  ...
  if ((rv = (*getfunctionlist)(&f)) != CKR_OK) {
    error("C_GetFunctionList for provider %s failed: %lu", provider_id, rv);
    goto fail;
  }
  p->function_list = f;
  if ((rv = f->C_Initialize(NULL)) != CKR_OK) {
    error("C_Initialize for provider %s failed: %lu", provider_id, rv);
    goto fail;
  }
  need_finalize = 1;
  if ((rv = f->C_GetInfo(&p->info)) != CKR_OK) {
    error("C_GetInfo for provider %s failed: %lu", provider_id, rv);
    goto fail;
  }
  ...
  debug("provider %s: manufacturerID <%s> cryptokiVersion %d.%d"
        ...
        provider_id, p->info.manufacturerID, p->info.cryptokiVersion.major,
        p->info.cryptokiVersion.minor, p->info.libraryDescription,
        p->info.libraryVersion.major, p->info.libraryVersion.minor);
  if ((rv = f->C_GetSlotList(CK_TRUE, NULL, &p->nslots)) != CKR_OK) {
    error("C_GetSlotList failed: %lu", rv);
    goto fail;
  }
  if (p->nslots == 0) {
    debug("%s: provider %s returned no slots", __func__, provider_id);
    ret = -SSH_PKCS11_ERR_NO_SLOTS;
    goto fail;
  }
  p->slotlist = xcalloc(p->nslots, sizeof(CK_SLOT_ID));
  if ((rv = f->C_GetSlotList(CK_TRUE, p->slotlist, &p->nslots)) != CKR_OK) {
    error("C_GetSlotList for provider %s failed: %lu", provider_id, rv);
    goto fail;
  }
  ...
  for (i = 0; i < p->nslots; i++) {
    ...
    if ((rv = f->C_GetTokenInfo(p->slotlist[i], token)) != CKR_OK) {
      error("C_GetTokenInfo for provider %s slot %lu "
            "failed: %lu",
            provider_id, (unsigned long)i, rv);
      continue;
    }
    ...
    debug("provider %s slot %lu: label <%s> manufacturerID <%s> "
          ...
          provider_id, (unsigned long)i, token->label, token->manufacturerID,
          token->model, token->serialNumber, token->flags);
    /*
     * open session, login with pin and retrieve public
     * keys (if keyp is provided)
     */
    if ((ret = pkcs11_open_session(p, i, pin, user)) != 0 || keyp == NULL)
      continue;
    pkcs11_fetch_keys(p, i, keyp, labelsp, &nkeys);
    pkcs11_fetch_certs(p, i, keyp, labelsp, &nkeys);
    if (nkeys == 0 && !p->slotinfo[i].logged_in && pkcs11_interactive) {
      /*
       * Some tokens require login before they will
       * expose keys.
       */
      if (pkcs11_login_slot(p, &p->slotinfo[i], CKU_USER) < 0) {
        error("login failed");
        continue;
      }
      pkcs11_fetch_keys(p, i, keyp, labelsp, &nkeys);
...
fail:
  if (need_finalize && (rv = f->C_Finalize(NULL)) != CKR_OK)
    error("C_Finalize for provider %s failed: %lu", provider_id, rv);
  if (p) {
    free(p->name);
    free(p->slotlist);
    free(p->slotinfo);
...
int pkcs11_add_provider(char *provider_id, char *pin, struct sshkey ***keyp,
  ...
  nkeys =
      pkcs11_register_provider(provider_id, pin, keyp, labelsp, &p, CKU_USER);
...
#ifdef WITH_PKCS11_KEYGEN
struct sshkey *pkcs11_gakp(char *provider_id, char *pin, unsigned int slotidx,
                           char *label, unsigned int type, unsigned int bits,
  ...
  *err = 0;
  ...
  else if ((ret = pkcs11_register_provider(provider_id, pin, NULL, NULL, &p,
                                           CKU_SO)) < 0) {
    debug("%s: could not register provider %s", __func__, provider_id);
    goto out;
  } else
    reset_provider = 1;
  ...
  if ((rv = f->C_SetOperationState(session, pin, strlen(pin), CK_INVALID_HANDLE,
                                   CK_INVALID_HANDLE)) != CKR_OK) {
  ...
  switch (type) {
  case KEY_RSA:
    if ((k = pkcs11_rsa_generate_private_key(p, slotidx, label, bits, keyid,
                                             err)) == NULL) {
    ...
    break;
  case KEY_ECDSA:
    if ((k = pkcs11_ecdsa_generate_private_key(p, slotidx, label, bits, keyid,
                                               err)) == NULL) {
...
struct sshkey *pkcs11_destroy_keypair(char *provider_id, char *pin,
  ...
  *err = 0;
  ...
  if (pkcs11_find(p, slotidx, attrs, nattrs, &obj) == 0 &&
    ...
    if ((rv = f->C_DestroyObject(session, obj)) != CKR_OK) {
      debug("%s: could not destroy private key 0x%hhx", __func__, keyid);
      *err = rv;
      goto out;
    }
  }
  ...
  if (pkcs11_find(p, slotidx, attrs, nattrs, &obj) == 0 &&
    ...
    /* get key type */
    nattrs = 0;
    FILL_ATTR(attrs, nattrs, CKA_KEY_TYPE, &key_type, sizeof(key_type));
    rv = f->C_GetAttributeValue(session, obj, attrs, nattrs);
    if (rv != CKR_OK) {
      debug("%s: could not get key type of public key 0x%hhx", __func__, keyid);
      *err = rv;
      key_type = -1;
    }
    if (key_type == CKK_RSA)
    ...
    if ((rv = f->C_DestroyObject(session, obj)) != CKR_OK) {
      debug("%s: could not destroy public key 0x%hhx", __func__, keyid);
      *err = rv;
      goto out;
    }
  }
...
void pkcs11_terminate(void) { error("%s: dlopen() not supported", __func__); }
#endif /* ENABLE_PKCS11 */
