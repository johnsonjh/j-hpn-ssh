...
static int process_config_line_depth(Options *options, struct passwd *pw,
                                     const char *host,
                                     const char *original_host, char *line,
                                     const char *filename, int linenum,
                                     int *activep, int flags, int depth);
...
void add_remote_forward(Options *options, const struct Forward *newfwd) {
  struct Forward *fwd;
  int i;
  ...
  options->remote_forwards =
      xreallocarray(options->remote_forwards, options->num_remote_forwards + 1,
...
static int match_cfg_line(Options *options, char **condition, struct passwd *pw,
                          ...
                          int post_canon, const char *filename, int linenum) {
  ...
  debug2("checking match for '%s' host %s originally %s", cp, host,
         original_host);
  while ((oattrib = attrib = strdelim(&cp)) && *attrib != '\0') {
    ...
    if (strcasecmp(attrib, "canonical") == 0) {
      ...
      debug3("%.200s line %d: %smatched '%s'", filename, linenum,
    ...
    if (strcasecmp(attrib, "host") == 0) {
      ...
      cmd = percent_expand(arg, "L", shorthost, "d", pw->pw_dir, "h", host, "l",
                           thishost, "n", original_host, "p", portstr, "r",
                           ruser, "u", pw->pw_name, "i", uidstr, (char *)NULL);
      if (result != 1) {
    ...
    debug3("%.200s line %d: %smatched '%s \"%.100s\"' ", filename, linenum,
...
static void rm_env(Options *options, const char *arg, const char *filename,
                   int linenum) {
  int i, j;
  char *cp;
  ...
  for (i = 0; i < options->num_send_env;) {
    ...
    debug3("%s line %d: removing environment %s", filename, linenum, cp);
    free(cp);
...
static const struct multistate multistate_strict_hostkey[] = {
    {"true", SSH_STRICT_HOSTKEY_YES},       {"false", SSH_STRICT_HOSTKEY_OFF},
    {"yes", SSH_STRICT_HOSTKEY_YES},        {"no", SSH_STRICT_HOSTKEY_OFF},
    {"ask", SSH_STRICT_HOSTKEY_ASK},        {"off", SSH_STRICT_HOSTKEY_OFF},
    {"accept-new", SSH_STRICT_HOSTKEY_NEW}, {NULL, -1}};
static const struct multistate multistate_yesnoaskconfirm[] = {
    {"true", 1}, {"false", 0},   {"yes", 1}, {"no", 0},
    {"ask", 2},  {"confirm", 3}, {NULL, -1}};
static const struct multistate multistate_addressfamily[] = {
    {"inet", AF_INET}, {"inet6", AF_INET6}, {"any", AF_UNSPEC}, {NULL, -1}};
static const struct multistate multistate_controlmaster[] = {
    {"true", SSHCTL_MASTER_YES},         {"yes", SSHCTL_MASTER_YES},
    {"false", SSHCTL_MASTER_NO},         {"no", SSHCTL_MASTER_NO},
    {"auto", SSHCTL_MASTER_AUTO},        {"ask", SSHCTL_MASTER_ASK},
    {"autoask", SSHCTL_MASTER_AUTO_ASK}, {NULL, -1}};
...
static const struct multistate multistate_canonicalizehostname[] = {
    {"true", SSH_CANONICALISE_YES},      {"false", SSH_CANONICALISE_NO},
    {"yes", SSH_CANONICALISE_YES},       {"no", SSH_CANONICALISE_NO},
    {"always", SSH_CANONICALISE_ALWAYS}, {NULL, -1}};

/*
 * Processes a single option line as used in the configuration files. This
 * only sets those values that have not already been set.
 */
int process_config_line(Options *options, struct passwd *pw, const char *host,
                        const char *original_host, char *line,
                        const char *filename, int linenum, int *activep,
                        int flags) {
  return process_config_line_depth(options, pw, host, original_host, line,
                                   filename, linenum, activep, flags, 0);
}
...
static int process_config_line_depth(Options *options, struct passwd *pw,
  ...
  case oIgnoredUnknownOption:
    debug("%s line %d: Ignored unknown option \"%s\"", filename, linenum,
          keyword);
    return 0;
  ...
  case oIdentityFile:
    ...
    if (*activep) {
      ...
      add_identity_file(options, NULL, arg, flags & SSHCONF_USERCONF);
    }
    break;
  ...
  case oCertificateFile:
    ...
    if (*activep) {
      ...
      add_certificate_file(options, arg, flags & SSHCONF_USERCONF);
    }
    break;
  ...
  case oProxyJump:
    ...
    if (parse_jump(s + len, options, *activep) == -1) {
      fatal("%.200s line %d: Invalid ProxyJump \"%s\"", filename, linenum,
            s + len);
    }
    return 0;
  ...
  case oPort:
    ...
    value = a2port(arg);
    if (value <= 0)
      fatal("%.200s line %d: Bad port '%s'.", filename, linenum, arg);
  ...
  case oCiphers:
    ...
    if (*arg != '-' && !ciphers_valid(*arg == '+' ? arg + 1 : arg))
      fatal("%.200s line %d: Bad SSH2 cipher spec '%s'.", filename, linenum,
            arg ? arg : "<NONE>");
  ...
  case oMacs:
    ...
    if (*arg != '-' && !mac_valid(*arg == '+' ? arg + 1 : arg))
      fatal("%.200s line %d: Bad SSH2 Mac spec '%s'.", filename, linenum,
            arg ? arg : "<NONE>");
  ...
  case oKexAlgorithms:
    ...
    if (*arg != '-' && !kex_names_valid(*arg == '+' ? arg + 1 : arg))
      fatal("%.200s line %d: Bad SSH2 KexAlgorithms '%s'.", filename, linenum,
            arg ? arg : "<NONE>");
  ...
  case oHost:
    ...
    *activep = 0;
    ...
    if (*activep)
      debug("%.200s line %d: Applying options for %.100s", filename, linenum,
            arg2);
  ...
  case oEscapeChar:
    ...
    if (strcmp(arg, "none") == 0)
      ...
      value = (u_char)arg[0];
    else if (arg[0] == '^' && arg[2] == 0 && (u_char)arg[1] >= 64 &&
             (u_char)arg[1] < 128)
    ...
    while ((arg = strdelim(&s)) != NULL && *arg != '\0') {
      ...
      if (*arg == '-') {
        ...
        options->send_env = xrecallocarray(
            options->send_env, options->num_send_env, options->num_send_env + 1,
  ...
  case oSetEnv:
    ...
    while ((arg = strdelimw(&s)) != NULL && *arg != '\0') {
      if (strchr(arg, '=') == NULL)
        fatal("%s line %d: Invalid SetEnv.", filename, linenum);
      if (!*activep || value != 0)
        continue;
      /* Adding a setenv var */
      if (options->num_setenv >= INT_MAX)
        fatal("%s line %d: too many SetEnv.", filename, linenum);
      options->setenv =
          xrecallocarray(options->setenv, options->num_setenv,
  ...
  case oControlPersist:
    ...
    if (strcmp(arg, "no") == 0 || strcmp(arg, "false") == 0)
      ...
      fatal("%.200s line %d: Bad ControlPersist argument.", filename, linenum);
    if (*activep && *intptr == -1) {
      *intptr = value;
  ...
  case oInclude:
    ...
    while ((arg = strdelim(&s)) != NULL && *arg != '\0') {
      ...
      if (*arg == '~' && (flags & SSHCONF_USERCONF) == 0)
        fatal("%.200s line %d: bad include path %s.", filename, linenum, arg);
      if (*arg != '/' && *arg != '~') {
        xasprintf(&arg2, "%s/%s",
                  (flags & SSHCONF_USERCONF) ? "~/" _PATH_SSH_USER_DIR : SSHDIR,
                  arg);
      } else
        arg2 = xstrdup(arg);
      ...
      if (r == GLOB_NOMATCH) {
        ...
        fatal("%.200s line %d: glob failed for %s.", filename, linenum, arg2);
      free(arg2);
      oactive = *activep;
      for (i = 0; i < (u_int)gl.gl_pathc; i++) {
        debug3("%.200s line %d: Including file %s "
               "depth %d%s",
               filename, linenum, gl.gl_pathv[i], depth,
               oactive ? "" : " (parse only)");
        r = read_config_file_depth(
            gl.gl_pathv[i], pw, host, original_host, options,
            flags | SSHCONF_CHECKPERM | (oactive ? 0 : SSHCONF_NEVERMATCH),
            activep, depth + 1);
        if (r != 1 && errno != ENOENT) {
  ...
  case oStreamLocalBindMask:
    arg = strdelim(&s);
    if (!arg || *arg == '\0')
      fatal("%.200s line %d: Missing StreamLocalBindMask argument.", filename,
            linenum);
  ...
  case oIdentityAgent:
    ...
    if (arg[0] == '$' && !valid_env_name(arg + 1)) {
      fatal("%.200s line %d: Invalid environment name %s.", filename, linenum,
            arg);
    }
    if (*activep && *charptr == NULL)
      *charptr = xstrdup(arg);
  ...
  if ((arg = strdelim(&s)) != NULL && *arg != '\0') {
    fatal("%.200s line %d: garbage at end of line; \"%.200s\".", filename,
          linenum, arg);
  }
  return 0;
}
...
void fill_default_options(Options *options) {
  ...
  if (options->clear_forwardings == -1)
    options->clear_forwardings = options->stdio_forward_host != NULL ? 1 : 0;
  if (options->clear_forwardings == 1)
  ...
  if (options->num_identity_files == 0) {
    ...
    add_identity_file(options, "~/", _PATH_SSH_CLIENT_ID_RAINBOW_I_CLASSIC, 0);
    add_identity_file(options, "~/", _PATH_SSH_CLIENT_ID_RAINBOW_III_CLASSIC,
                      0);
    add_identity_file(options, "~/", _PATH_SSH_CLIENT_ID_RAINBOW_V_CLASSIC, 0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_SPHINCS_HARAKA_128F_ROBUST, 0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_SPHINCS_SHA256_128F_ROBUST, 0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_SPHINCS_SHAKE256_128F_ROBUST, 0);
///// OQS_TEMPLATE_FRAGMENT_ADD_PQ_ID_FILES_END
    ...
    add_identity_file(options, "~/", _PATH_SSH_CLIENT_ID_RSA3072_DILITHIUM_2,
                      0);
    ...
    add_identity_file(options, "~/", _PATH_SSH_CLIENT_ID_RSA3072_PICNIC_L1FULL,
                      0);
    ...
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_RSA3072_RAINBOW_I_CLASSIC, 0);
    add_identity_file(options, "~/", _PATH_SSH_CLIENT_ID_P256_RAINBOW_I_CLASSIC,
                      0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_P384_RAINBOW_III_CLASSIC, 0);
    add_identity_file(options, "~/", _PATH_SSH_CLIENT_ID_P521_RAINBOW_V_CLASSIC,
                      0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_RSA3072_SPHINCS_HARAKA_128F_ROBUST,
                      0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_P256_SPHINCS_HARAKA_128F_ROBUST, 0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_RSA3072_SPHINCS_SHA256_128F_ROBUST,
                      0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_P256_SPHINCS_SHA256_128F_ROBUST, 0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_RSA3072_SPHINCS_SHAKE256_128F_ROBUST,
                      0);
    add_identity_file(options, "~/",
                      _PATH_SSH_CLIENT_ID_P256_SPHINCS_SHAKE256_128F_ROBUST, 0);
...
#define CLEAR_ON_NONE(v)                                                       \
  ...
  if (options->jump_host != NULL && strcmp(options->jump_host, "none") == 0 &&
 ...
 */
static int parse_fwd_field(char **p, struct fwdarg *fwd) {
  char *ep, *cp = *p;
  int ispath = 0;
...
int parse_forward(struct Forward *fwd, const char *fwdspec, int dynamicfwd,
  ...
  if (dynamicfwd) {
    ...
    if (!(i == 3 || i == 4)) {
      if (fwd->connect_path == NULL && fwd->listen_path == NULL)
        goto fail_free;
    }
...
static const char *fmt_intarg(OpCodes code, int val) {
  if (val == -1)
    return "unset";
  switch (code) {
  case oAddressFamily:
...
static void dump_cfg_string(OpCodes code, const char *val) {
  if (val == NULL)
    return;
  printf("%s %s\n", lookup_opcode_name(code), val);
}

static void dump_cfg_strarray(OpCodes code, u_int count, char **vals) {
  u_int i;
...
static void dump_cfg_forwards(OpCodes code, u_int count,
                              const struct Forward *fwds) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (fwd->listen_port == PORT_STREAMLOCAL)
      ...
      printf(" [%s]:%d", fwd->listen_host, fwd->listen_port);
    }
    if (code != oDynamicForward) {
      if (fwd->connect_port == PORT_STREAMLOCAL)
        ...
        printf(" [%s]:%d", fwd->connect_host, fwd->connect_port);
      }
    }
    printf("\n");
  }
...
void dump_client_config(Options *o, const char *host) {
  ...
  all_key = sshkey_alg_list(0, 0, 1, ',');
  if (kex_assemble_names(&o->hostkeyalgorithms, KEX_DEFAULT_PK_ALG, all_key) !=
      0)
  ...
  dump_cfg_fmtint(oKbdInteractiveAuthentication,
                  o->kbd_interactive_authentication);
  dump_cfg_fmtint(oNoHostAuthenticationForLocalhost,
  ...
  dump_cfg_string(oKexAlgorithms,
                  o->kex_algorithms ? o->kex_algorithms : KEX_CLIENT_KEX);
  ...
  dump_cfg_strarray_oneline(oCanonicalDomains, o->num_canonical_domains,
                            o->canonical_domains);
  dump_cfg_strarray(oCertificateFile, o->num_certificate_files,
                    o->certificate_files);
  dump_cfg_strarray_oneline(oGlobalKnownHostsFile, o->num_system_hostfiles,
                            o->system_hostfiles);
  dump_cfg_strarray_oneline(oUserKnownHostsFile, o->num_user_hostfiles,
                            o->user_hostfiles);
  ...
  if (o->jump_host == NULL)
    ...
    printf("proxyjump %s%s%s%s%s%s%s%s%s\n",
           ...
           o->jump_port <= 0 ? "" : ":", o->jump_port <= 0 ? "" : buf);
  }
}
