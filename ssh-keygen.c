...
#define CERTOPT_DEFAULT                                                        \
  (CERTOPT_X_FWD | CERTOPT_AGENT_FWD | CERTOPT_PORT_FWD | CERTOPT_PTY |        \
   CERTOPT_USER_RC)
...
enum { FMT_RFC4716, FMT_PKCS8, FMT_PEM } convert_format = FMT_RFC4716;
...
static void type_bits_valid(int type, const char *name, u_int32_t *bitsp) {
  if (type == KEY_UNSPEC)
  ...
  if (*bitsp == 0) {
    ...
    case KEY_DSA:
      *bitsp = DEFAULT_BITS_DSA;
      break;
    case KEY_ECDSA:
      if (name != NULL && (nid = sshkey_ecdsa_nid_from_name(name)) > 0)
...
#endif
    case KEY_DILITHIUM:
      if (name != NULL && (variant = sshkey_variant_from_name(name)) > 0)
  ...
  case KEY_DILITHIUM:
    if (sshkey_dilithium_bits_to_variant(*bitsp) == -1)
      fatal("Invalid dilithium key length: valid lengths are 11096, 16352, "
            "21608 or 26928 bits");
    break;
#ifdef WITH_OPENSSL
  case KEY_DSA:
...
#ifdef WITH_OPENSSL
static void do_convert_to_ssh2(struct passwd *pw, struct sshkey *k) {
  struct sshbuf *b;
  char comment[61], *b64;
  int r;
  ...
  snprintf(comment, sizeof(comment),
           "%u-bit %s, converted by %s@%s from OpenSSH", sshkey_size(k),
           sshkey_type(k), pw->pw_name, hostname);
...
static void do_convert_to_pkcs8(struct sshkey *k) {
...
static void do_convert_to(struct passwd *pw) {
  struct sshkey *k;
  struct stat st;
  int r;
 ...
 * instead of 16.
 */
static void buffer_get_bignum_bits(struct sshbuf *b, BIGNUM *value) {
  ...
  if (sshbuf_len(b) < bytes)
    fatal("%s: input buffer too small: need %d have %zu", __func__, bytes,
          sshbuf_len(b));
...
static struct sshkey *do_convert_private_ssh2(struct sshbuf *b) {
  ...
  if ((r = sshbuf_get_u32(b, &i1)) != 0 ||
      ...
      (r = sshbuf_get_u32(b, &i2)) != 0 || (r = sshbuf_get_u32(b, &i3)) != 0 ||
  ...
  case KEY_RSA:
    ...
    if ((rsa_n = BN_new()) == NULL || (rsa_d = BN_new()) == NULL ||
  ...
  if (sshkey_sign(key, &sig, &slen, data, sizeof(data), NULL, NULL, 0) != 0 ||
      sshkey_verify(key, sig, slen, data, sizeof(data), NULL, 0, NULL) != 0) {
    sshkey_free(key);
    free(sig);
    return NULL;
  }
...
static int get_line(FILE *fp, char *line, size_t len) {
  int c;
  size_t pos = 0;
...
static void do_convert_from_ssh2(struct passwd *pw, struct sshkey **k,
                                 int *private) {
  ...
  while ((blen = get_line(fp, line, sizeof(line))) != -1) {
    if (blen > 0 && line[blen - 1] == '\\')
      escaped++;
    if (strncmp(line, "----", 4) == 0 || strstr(line, ": ") != NULL) {
  ...
  len = strlen(encoded);
  if (((len % 4) == 3) && (encoded[len - 1] == '=') &&
      (encoded[len - 2] == '=') && (encoded[len - 3] == '='))
    encoded[len - 3] = '\0';
...
static void do_convert_from_pkcs8(struct sshkey **k, int *private) {
  EVP_PKEY *pubkey;
  FILE *fp;
...
#endif
  default:
    fatal("%s: unsupported pubkey type %d", __func__, EVP_PKEY_base_id(pubkey));
  }
...
static void do_convert_from(struct passwd *pw) {
...
#ifdef OPENSSL_HAS_ECC
    case KEY_ECDSA:
      ok = PEM_write_ECPrivateKey(stdout, k->ecdsa, NULL, NULL, 0, NULL, NULL);
      break;
#endif
    case KEY_RSA:
      ok = PEM_write_RSAPrivateKey(stdout, k->rsa, NULL, NULL, 0, NULL, NULL);
      break;
    default:
      fatal("%s: unsupported key type %s", __func__, sshkey_type(k));
    }
...
static void do_print_public(struct passwd *pw) {
  struct sshkey *prv;
  struct stat st;
  int r;
  char *comment = NULL;
...
static void do_download(struct passwd *pw) {
  ...
  for (i = 0; i < nkeys; i++) {
    if (print_fingerprint) {
      ...
      ra = sshkey_fingerprint(keys[i], fingerprint_hash, SSH_FP_RANDOMART);
      if (fp == NULL || ra == NULL)
        fatal("%s: sshkey_fingerprint fail", __func__);
      printf("%u %s %s (PKCS11 key)\n", sshkey_size(keys[i]), fp,
             sshkey_type(keys[i]));
...
static struct sshkey *try_read_key(char **cpp) {
  struct sshkey *ret;
  int r;
...
static void fingerprint_one_key(const struct sshkey *public,
                                const char *comment) {
...
static void fingerprint_private(const char *path) {
  struct stat st;
  char *comment = NULL;
  ...
  if ((r = sshkey_load_public(path, &public, &comment)) != 0) {
    ...
    if ((r = sshkey_load_private(path, NULL, &public, &comment)) != 0) {
...
static void do_gen_all_hostkeys(struct passwd *pw) {
  ...
  public
    ...
    xasprintf(&prv_file, "%s%s", identity_file, key_types[i].path);
    ...
    if (stat(prv_file, &st) == 0) {
      ...
      error("Could not stat %s: %s", key_types[i].path, strerror(errno));
      goto failnext;
    }
     ...
     * key generation.
     */
    xasprintf(&prv_tmp, "%s%s.XXXXXXXXXX", identity_file, key_types[i].path);
    xasprintf(&pub_tmp, "%s%s.pub.XXXXXXXXXX", identity_file,
              key_types[i].path);
    xasprintf(&pub_file, "%s%s.pub", identity_file, key_types[i].path);
    ...
    if ((fd = mkstemp(prv_tmp)) == -1) {
      error("Could not save your public key in %s: %s", prv_tmp,
            strerror(errno));
      goto failnext;
    }
    ...
    if ((r = sshkey_save_private(private, prv_tmp, "", comment,
                                 private_key_format, openssh_format_cipher,
                                 rounds)) != 0) {
      error("Saving key \"%s\" failed: %s", prv_tmp, ssh_err(r));
      goto failnext;
    }
    if ((fd = mkstemp(pub_tmp)) == -1) {
      error("Could not save your public key in %s: %s", pub_tmp,
            strerror(errno));
      goto failnext;
    }
    (void)fchmod(fd, 0644);
    ...
    if (rename(pub_tmp, pub_file) != 0) {
      error("Unable to move %s into position: %s", pub_file, strerror(errno));
      goto failnext;
    }
    if (rename(prv_tmp, prv_file) != 0) {
      error("Unable to move %s into position: %s", key_types[i].path,
            strerror(errno));
    failnext:
      first = 0;
      goto next;
...
static int known_hosts_hash(struct hostkey_foreach_line *l, void *_ctx) {
...
static void do_known_hosts(struct passwd *pw, const char *name, int find_host,
  ...
  if ((r = hostkeys_foreach(identity_file,
                            (find_host || !hash_hosts) ? known_hosts_find_delete
                                                       : known_hosts_hash,
  ...
  if (ctx.invalid) {
    ...
    if (link(identity_file, old) == -1)
      fatal("link %.100s to %.100s: %s", identity_file, old, strerror(errno));
    /* Move new one into place */
    if (rename(tmp, identity_file) == -1) {
      error("rename\"%s\" to \"%s\": %s", tmp, identity_file, strerror(errno));
      unlink(tmp);
      unlink(old);
      exit(1);
    }
...
static void do_change_passphrase(struct passwd *pw) {
  ...
  if (r == SSH_ERR_KEY_WRONG_PASSPHRASE) {
    if (identity_passphrase)
      ...
      old_passphrase =
          read_passphrase("Enter old passphrase: ", RP_ALLOW_STDIN);
    r = sshkey_load_private(identity_file, old_passphrase, &private, &comment);
  ...
  if (identity_new_passphrase) {
    ...
    passphrase1 = read_passphrase("Enter new passphrase (empty for no "
                                  "passphrase): ",
                                  RP_ALLOW_STDIN);
    passphrase2 =
        read_passphrase("Enter same passphrase again: ", RP_ALLOW_STDIN);
  ...
  if ((r = sshkey_save_private(private, identity_file, passphrase1, comment,
                               private_key_format, openssh_format_cipher,
                               rounds)) != 0) {
    error("Saving key \"%s\" failed: %s.", identity_file, ssh_err(r));
 ...
 * Print the SSHFP RR.
 */
static int do_print_resource_record(struct passwd *pw, char *fname, char *hname,
                                    int print_generic) {
  ...
  if ((r = sshkey_load_public(fname, &public, &comment)) != 0)
    fatal("Failed to read v2 public key from \"%s\": %s.", fname, ssh_err(r));
 ...
 */
static void do_change_comment(struct passwd *pw, const char *identity_comment) {
  ...
  if (stat(identity_file, &st) == -1)
    fatal("%s: %s", identity_file, strerror(errno));
  if ((r = sshkey_load_private(identity_file, "", &private, &comment)) == 0)
    passphrase = xstrdup("");
  else if (r != SSH_ERR_KEY_WRONG_PASSPHRASE)
    fatal("Cannot load private key \"%s\": %s.", identity_file, ssh_err(r));
  else {
    if (identity_passphrase)
      ...
      passphrase = read_passphrase("Enter passphrase: ", RP_ALLOW_STDIN);
    /* Try to load using the passphrase. */
    if ((r = sshkey_load_private(identity_file, passphrase, &private,
                                 &comment)) != 0) {
  ...
  /* Save the file using the new passphrase. */
  if ((r = sshkey_save_private(private, identity_file, passphrase, new_comment,
                               private_key_format, openssh_format_cipher,
                               rounds)) != 0) {
    error("Saving key \"%s\" failed: %s", identity_file, ssh_err(r));
    freezero(passphrase, strlen(passphrase));
...
static void add_flag_option(struct sshbuf *c, const char *name) {
  int r;
...
static void add_string_option(struct sshbuf *c, const char *name,
                              const char *value) {
  struct sshbuf *b;
  int r;
...
static void prepare_options_buf(struct sshbuf *c, int which) {
  size_t i;

  sshbuf_reset(c);
  if ((which & OPTIONS_CRITICAL) != 0 && certflags_command != NULL)
  ...
  if ((which & OPTIONS_EXTENSIONS) != 0 && (certflags_flags & CERTOPT_PTY) != 0)
  ...
  if ((which & OPTIONS_CRITICAL) != 0 && certflags_src_addr != NULL)
  ...
  for (i = 0; i < ncert_userext; i++) {
    ...
    if (cert_userext[i].val == NULL)
      ...
      add_string_option(c, cert_userext[i].key, cert_userext[i].val);
    }
  }
}
...
static struct sshkey *load_pkcs11_key(char *path) {
  ...
  if ((r = sshkey_load_public(path, &public, NULL)) != 0)
    fatal("Couldn't load CA public key \"%s\": %s", path, ssh_err(r));

  nkeys = pkcs11_add_provider(pkcs11provider, identity_passphrase, &keys, NULL);
...
static int agent_signer(struct sshkey *key, u_char **sigp, size_t *lenp,
                        ...
                        const char *provider, u_int compat, void *ctx) {
...
static void do_ca_sign(struct passwd *pw, const char *ca_key_path,
  ...
  if (pkcs11provider != NULL) {
    ...
    if ((r = sshkey_load_public(tmp, &ca, NULL)) != 0)
      fatal("Cannot load CA public key %s: %s", tmp, ssh_err(r));
    if ((r = ssh_get_authentication_socket(&agent_fd)) != 0)
      fatal("Cannot use public key for CA signature: %s", ssh_err(r));
  ...
  if (key_type_name != NULL) {
    if (sshkey_type_from_name(key_type_name) != ca->type) {
      fatal("CA key type %s doesn't match specified %s", sshkey_ssh_name(ca),
            key_type_name);
    }
  ...
  for (i = 0; i < argc; i++) {
    ...
    if ((r = sshkey_load_public(tmp, &public, &comment)) != 0)
      fatal("%s: unable to open \"%s\": %s", __func__, tmp, ssh_err(r));
    if (sshkey_is_cert(public))
      fatal("%s: key \"%s\" type %s cannot be certified", __func__, tmp,
            sshkey_type(public));

    /* Prepare certificate to sign */
    if ((r = sshkey_to_certified(public)) != 0)
      fatal("Could not upgrade key %s to certificate: %s", tmp, ssh_err(r));
  public
    ->cert->type = cert_key_type;
  public
    ->cert->serial = (u_int64_t)cert_serial;
  public
    ->cert->key_id = xstrdup(cert_key_id);
  public
    ->cert->nprincipals = n;
  public
    ->cert->principals = plist;
  public
    ->cert->valid_after = cert_valid_from;
  public
    ->cert->valid_before = cert_valid_to;
    prepare_options_buf(public->cert->critical, OPTIONS_CRITICAL);
    prepare_options_buf(public->cert->extensions, OPTIONS_EXTENSIONS);
    ...
    if (agent_fd != -1 && (ca->flags & SSHKEY_FLAG_EXT) != 0) {
      if ((r = sshkey_certify_custom(public, ca, key_type_name, sk_provider,
        ...
        fatal("Couldn't certify key %s via agent: %s", tmp, ssh_err(r));
    } else {
      if (sshkey_is_sk(ca) && (ca->sk_flags & SSH_SK_USER_PRESENCE_REQD)) {
        notifier = notify_start(0, "Confirm user presence for key %s %s",
                                sshkey_type(ca), ca_fp);
      }
      r = sshkey_certify(public, ca, key_type_name, sk_provider);
    ...
    if ((fd = open(out, O_WRONLY | O_CREAT | O_TRUNC, 0644)) == -1)
      fatal("Could not open \"%s\" for writing: %s", out, strerror(errno));
    if ((f = fdopen(fd, "w")) == NULL)
      fatal("%s: fdopen: %s", __func__, strerror(errno));
    if ((r = sshkey_write(public, f)) != 0)
      fatal("Could not write certified key to %s: %s", out, ssh_err(r));
    fprintf(f, " %s\n", comment);
    fclose(f);
    ...
    if (!quiet) {
      ...
      logit("Signed %s key %s: id \"%s\" serial %llu%s%s "
            ...
            cert_principals != NULL ? cert_principals : "", valid);
    }
...
static u_int64_t parse_relative_time(const char *s, time_t now) {
  int64_t mul, secs;
...
static void parse_cert_times(char *timespec) {
  char *from, *to;
...
static void add_cert_option(char *opt) {
  char *val, *cp;
  int iscrit = 0;
  ...
  if (strcasecmp(opt, "clear") == 0)
    ...
    if ((cp = strchr(val, '=')) != NULL)
      *cp++ = '\0';
    cert_userext =
        xreallocarray(cert_userext, ncert_userext + 1, sizeof(*cert_userext));
    cert_userext[ncert_userext].key = val;
    cert_userext[ncert_userext].val = cp == NULL ? NULL : xstrdup(cp);
...
static void show_options(struct sshbuf *optbuf, int in_critical) {
  ...
  while (sshbuf_len(options) != 0) {
    ...
    if ((r = sshbuf_get_cstring(options, &name, NULL)) != 0 ||
      ...
      fatal("%s: buffer error: %s", __func__, ssh_err(r));
    printf("                %s", name);
    if (!in_critical && (strcmp(name, "permit-X11-forwarding") == 0 ||
      ...
      printf("\n");
    } else if (in_critical && (strcmp(name, "force-command") == 0 ||
...
static void do_show_cert(struct passwd *pw) {
  ...
  while (getline(&line, &linesize, f) != -1) {
    ...
    if ((r = sshkey_read(key, &cp)) != 0) {
      error("%s:%lu: invalid key: %s", path, lnum, ssh_err(r));
      continue;
    }
...
static void load_krl(const char *path, struct ssh_krl **krlp) {
  ...
  /* XXX check sigs */
  if ((r = ssh_krl_from_blob(krlbuf, krlp, NULL, 0)) != 0 || *krlp == NULL)
    fatal("Invalid KRL file: %s", ssh_err(r));
  sshbuf_free(krlbuf);
}

static void hash_to_blob(const char *cp, u_char **blobp, size_t *lenp,
...
static void update_krl_from_file(struct passwd *pw, const char *file,
                                 int wild_ca, const struct sshkey *ca,
                                 struct ssh_krl *krl) {
  struct sshkey *key = NULL;
  u_long lnum = 0;
  ...
  while (getline(&line, &linesize, krl_spec) != -1) {
    ...
    if (strncasecmp(cp, "serial:", 7) == 0) {
      ...
      if (*cp == '\0' || (*ep != '\0' && *ep != '-'))
        fatal("%s:%lu: invalid serial \"%s\"", path, lnum, cp);
      if (errno == ERANGE && serial == ULLONG_MAX)
        fatal("%s:%lu: serial out of range", path, lnum);
      serial2 = serial;
      if (*ep == '-') {
        cp = ep + 1;
        errno = 0;
        ...
        if (serial2 <= serial)
          fatal("%s:%lu: invalid serial range "
                "%llu:%llu",
                path, lnum, (unsigned long long)serial,
                (unsigned long long)serial2);
      }
      if (ssh_krl_revoke_cert_by_serial_range(krl, ca, serial, serial2) != 0) {
        fatal("%s: revoke serial failed", __func__);
      }
      ...
      if (r != 0)
        fatal("%s: revoke key failed: %s", __func__, ssh_err(r));
    } else {
      ...
      if ((key = sshkey_new(KEY_UNSPEC, -1)) == NULL)
        fatal("sshkey_new");
      if ((r = sshkey_read(key, &cp)) != 0)
        fatal("%s:%lu: invalid key: %s", path, lnum, ssh_err(r));
      if (was_explicit_key)
        r = ssh_krl_revoke_key_explicit(krl, key);
      else if (was_sha1) {
        if (sshkey_fingerprint_raw(key, SSH_DIGEST_SHA1, &blob, &blen) != 0) {
          fatal("%s:%lu: fingerprint failed", file, lnum);
        }
        r = ssh_krl_revoke_key_sha1(krl, blob, blen);
      } else if (was_sha256) {
        if (sshkey_fingerprint_raw(key, SSH_DIGEST_SHA256, &blob, &blen) != 0) {
          fatal("%s:%lu: fingerprint failed", file, lnum);
        }
        r = ssh_krl_revoke_key_sha256(krl, blob, blen);
      } else
...
static void do_gen_krl(struct passwd *pw, int updating, const char *ca_key_path,
  ...
  if (stat(identity_file, &sb) == -1) {
    if (errno != ENOENT)
      fatal("Cannot access KRL \"%s\": %s", identity_file, strerror(errno));
    if (updating)
...
static void do_check_krl(struct passwd *pw, int print_krl, int argc,
                         char **argv) {
  int i, r, ret = 0;
  char *comment;
  struct ssh_krl *krl;
  ...
  for (i = 0; i < argc; i++) {
    if ((r = sshkey_load_public(argv[i], &k, &comment)) != 0)
      fatal("Cannot load public key %s: %s", argv[i], ssh_err(r));
    r = ssh_krl_check_key(krl, k);
    printf("%s%s%s%s: %s\n", argv[i], *comment ? " (" : "", comment,
           *comment ? ")" : "", r == 0 ? "ok" : "REVOKED");
    if (r != 0)
      ret = 1;
    sshkey_free(k);
    free(comment);
...
static struct sshkey *load_sign_key(const char *keypath,
                                    const struct sshkey *pubkey) {
  ...
  for (i = 0; suffixes[i]; i++) {
    ...
    if (plen <= slen || strcmp(privpath + plen - slen, suffixes[i]) != 0)
      continue;
    privpath[plen - slen] = '\0';
    debug("%s: %s looks like a public key, using private key "
          "path %s instead",
          __func__, keypath, privpath);
  }
  ...
  if (!sshkey_equal_public(pubkey, privkey)) {
    error("Public key %s doesn't match private %s", keypath, privpath);
    goto done;
  }
...
static int sign_one(struct sshkey *signkey, const char *filename, int fd,
                    const char *sig_namespace, sshsig_signer *signer,
                    void *signer_ctx) {
  ...
  if (signer == NULL && sshkey_is_sk(signkey) &&
    ...
    if ((fp = sshkey_fingerprint(signkey, fingerprint_hash, SSH_FP_DEFAULT)) ==
        NULL)
      fatal("%s: sshkey_fingerprint failed", __func__);
  ...
  if (fd == STDIN_FILENO) {
    ...
    if (confirm_overwrite(wfile)) {
      if ((wfd = open(wfile, O_WRONLY | O_CREAT | O_TRUNC, 0666)) == -1) {
        oerrno = errno;
        error("Cannot open %s: %s", wfile, strerror(errno));
        errno = oerrno;
        r = SSH_ERR_SYSTEM_ERROR;
        goto out;
      }
      if (atomicio(vwrite, wfd, asig, strlen(asig)) != strlen(asig)) {
        oerrno = errno;
        error("Cannot write to %s: %s", wfile, strerror(errno));
        errno = oerrno;
        r = SSH_ERR_SYSTEM_ERROR;
        goto out;
      }
...
static int sig_sign(const char *keypath, const char *sig_namespace, int argc,
                    char **argv) {
  int i, fd = -1, r, ret = -1;
  int agent_fd = -1;
  ...
  if (argc == 0) {
    if ((r = sign_one(signkey, "(stdin)", STDIN_FILENO, sig_namespace, signer,
                      &agent_fd)) != 0)
      goto done;
  } else {
    for (i = 0; i < argc; i++) {
      if (strcmp(argv[i], "-") == 0)
        ...
        error("Cannot open %s for signing: %s", argv[i], strerror(errno));
        goto done;
      }
      if ((r = sign_one(signkey, argv[i], fd, sig_namespace, signer,
                        &agent_fd)) != 0)
        goto done;
      if (fd != STDIN_FILENO)
        close(fd);
...
static int sig_verify(const char *signature, const char *sig_namespace,
                      const char *principal, const char *allowed_keys,
  ...
  if ((r = sshsig_verify_fd(sigbuf, STDIN_FILENO, sig_namespace, &sign_key,
                            &sig_details)) != 0)
    goto done; /* sshsig_verify() prints error */

  if ((fp = sshkey_fingerprint(sign_key, fingerprint_hash, SSH_FP_DEFAULT)) ==
      NULL)
    fatal("%s: sshkey_fingerprint failed", __func__);
  debug("Valid (unverified) signature from key %s", fp);
  if (sig_details != NULL) {
    debug2("%s: signature details: counter = %u, flags = 0x%02x", __func__,
  ...
  if (allowed_keys != NULL &&
      (r = sshsig_check_allowed_keys(allowed_keys, sign_key, principal,
                                     sig_namespace)) != 0) {
    ...
    if (ret == 0) {
      ...
      if (principal == NULL) {
        ...
        printf("Good \"%s\" signature for %s with %s key %s\n", sig_namespace,
               principal, sshkey_type(sign_key), fp);
      }
    } else {
...
static int sig_find_principals(const char *signature,
  ...
  if ((r = sshsig_get_pubkey(sigbuf, &sign_key)) != 0) {
    error("%s: sshsig_get_pubkey: %s", __func__, ssh_err(r));
    goto done;
  }
  if ((r = sshsig_find_principals(allowed_keys, sign_key, &principals)) != 0) {
    error("%s: sshsig_get_principal: %s", __func__, ssh_err(r));
    goto done;
  }
  ret = 0;
done:
...
static void do_moduli_gen(const char *out_file, char **opts, size_t nopts) {
#ifdef WITH_OPENSSL
  ...
  for (i = 0; i < nopts; i++) {
    if (strncmp(opts[i], "memory=", 7) == 0) {
      memory = (u_int32_t)strtonum(opts[i] + 7, 1, UINT_MAX, &errstr);
      if (errstr) {
        fatal("Memory limit is %s: %s", errstr, opts[i] + 7);
      }
      ...
      moduli_bits = (int)strtonum(opts[i] + 5, 1, INT_MAX, &errstr);
      if (errstr) {
        fatal("Invalid number: %s (%s)", opts[i] + 12, errstr);
      }
    } else {
  ...
  if ((out = fopen(out_file, "w")) == NULL) {
    fatal("Couldn't open modulus candidate file \"%s\": %s", out_file,
          strerror(errno));
  }
...
static void do_moduli_screen(const char *out_file, char **opts, size_t nopts) {
#ifdef WITH_OPENSSL
  /* Moduli generation/screening */
  ...
  for (i = 0; i < nopts; i++) {
    if (strncmp(opts[i], "lines=", 6) == 0) {
      ...
      generator_wanted =
          (u_int32_t)strtonum(opts[i] + 10, 1, UINT_MAX, &errstr);
      if (errstr != NULL) {
        fatal("Generator invalid: %s (%s)", opts[i] + 10, errstr);
      }
    } else if (strncmp(opts[i], "prime-tests=", 12) == 0) {
      prime_tests = (int)strtonum(opts[i] + 12, 1, INT_MAX, &errstr);
      if (errstr) {
        fatal("Invalid number: %s (%s)", opts[i] + 12, errstr);
      }
    } else {
  ...
  if ((out = fopen(out_file, "a")) == NULL) {
    fatal("Couldn't open moduli file \"%s\": %s", out_file, strerror(errno));
  }
  setvbuf(out, NULL, _IOLBF, 0);
  if (prime_test(in, out, prime_tests == 0 ? 100 : prime_tests,
                 generator_wanted, checkpoint, start_lineno,
                 lines_to_process) != 0)
...
static char *private_key_passphrase(void) {
  ...
  passphrase_again:
    passphrase1 = read_passphrase("Enter passphrase (empty for no "
                                  "passphrase): ",
                                  RP_ALLOW_STDIN);
    passphrase2 =
        read_passphrase("Enter same passphrase again: ", RP_ALLOW_STDIN);
...
static const char *skip_ssh_url_preamble(const char *s) {
...
static int do_download_sk(const char *skprovider, const char *device) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if ((r = sshsk_load_resident(skprovider, device, pin, &keys, &nkeys)) !=
        0) {
  ...
  for (i = 0; i < nkeys; i++) {
    if (keys[i]->type != KEY_ECDSA_SK && keys[i]->type != KEY_ED25519_SK) {
      error("Unsupported key type %s (%d)", sshkey_type(keys[i]),
            keys[i]->type);
      continue;
    }
    if ((fp = sshkey_fingerprint(keys[i], fingerprint_hash, SSH_FP_DEFAULT)) ==
        NULL)
      fatal("%s: sshkey_fingerprint failed", __func__);
    debug("%s: key %zu: %s %s %s (flags 0x%02x)", __func__, i,
          sshkey_type(keys[i]), fp, keys[i]->sk_application, keys[i]->sk_flags);
    ...
    if ((r = sshkey_save_private(keys[i], path, pass, keys[i]->sk_application,
                                 private_key_format, openssh_format_cipher,
                                 rounds)) != 0) {
      error("Saving key \"%s\" failed: %s", path, ssh_err(r));
      free(path);
      break;
    }
    if (!quiet) {
      printf("Saved %s key%s%s to %s\n", sshkey_type(keys[i]),
             *ext != '\0' ? " " : "",
             *ext != '\0' ? keys[i]->sk_application : "", path);
    }

    /* Save public key too */
    xasprintf(&pubpath, "%s.pub", path);
    free(path);
    if ((r = sshkey_save_public(keys[i], pubpath, keys[i]->sk_application)) !=
        0) {
      error("Saving public key \"%s\" failed: %s", pubpath, ssh_err(r));
      free(pubpath);
      break;
    }
    free(pubpath);
...
static void usage(void) {
  fprintf(stderr,
          ...
          "[-m format]\n"
          "                  [-t dsa | ecdsa | ecdsa-sk | ed25519 | ed25519-sk "
          "| rsa]\n"
...
#endif
      "       ssh-keygen -I certificate_identity -s ca_key [-hU] [-D "
      "pkcs11_provider]\n"
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv,
    ...
    case 'A':
      ...
      break;
    case 'b':
      bits = (u_int32_t)strtonum(optarg, 1, UINT32_MAX, &errstr);
      if (errstr)
        fatal("Bits has bad value %s (%s)", optarg, errstr);
      break;
    case 'E':
    ...
    case 'Q':
      ...
      break;
    case 'O':
      opts = xrecallocarray(opts, nopts, nopts + 1, sizeof(*opts));
    ...
    case 'v':
      if (log_level == SYSLOG_LEVEL_INFO)
        ...
        if (log_level >= SYSLOG_LEVEL_DEBUG1 && log_level < SYSLOG_LEVEL_DEBUG3)
          log_level++;
      }
      break;
    case 'r':
    ...
    case 'a':
      ...
      if (errstr)
        fatal("Invalid number: %s (%s)", optarg, errstr);
      break;
    case 'V':
  ...
  if (sign_op != NULL) {
    if (strncmp(sign_op, "find-principals", 15) == 0) {
      ...
      return sig_sign(identity_file, cert_principals, argc, argv);
      ...
      return sig_verify(ca_key_path, cert_principals, NULL, NULL, NULL);
      ...
      return sig_verify(ca_key_path, cert_principals, cert_key_id,
  ...
  } else if (argc > 0 && !gen_krl && !check_krl && !do_gen_candidates &&
  ...
  if (gen_krl) {
    do_gen_krl(pw, update_krl, ca_key_path, cert_serial, identity_comment, argc,
               argv);
    return (0);
  }
  if (check_krl) {
  ...
  if (ca_key_path != NULL) {
    ...
    do_ca_sign(pw, ca_key_path, prefer_agent, cert_serial, cert_serial_autoinc,
               argc, argv);
  }
  if (show_cert)
    do_show_cert(pw);
  ...
  if (rr_hostname != NULL) {
    ...
    if (have_identity) {
      n = do_print_resource_record(pw, identity_file, rr_hostname,
                                   print_generic);
      if (n == 0)
        fatal("%s: %s", identity_file, strerror(errno));
      exit(0);
      ...
      n += do_print_resource_record(pw, _PATH_HOST_RSA_KEY_FILE, rr_hostname,
                                    print_generic);
      n += do_print_resource_record(pw, _PATH_HOST_DSA_KEY_FILE, rr_hostname,
                                    print_generic);
      n += do_print_resource_record(pw, _PATH_HOST_ECDSA_KEY_FILE, rr_hostname,
                                    print_generic);
      n += do_print_resource_record(pw, _PATH_HOST_ED25519_KEY_FILE,
                                    rr_hostname, print_generic);
      n += do_print_resource_record(pw, _PATH_HOST_XMSS_KEY_FILE, rr_hostname,
                                    print_generic);
      if (n == 0)
        fatal("no keys found.");
      exit(0);
  ...
  case KEY_ED25519_SK:
    for (i = 0; i < nopts; i++) {
      if (strcasecmp(opts[i], "no-touch-required") == 0) {
        ...
        if ((r = sshbuf_load_file(opts[i] + 10, &challenge)) != 0) {
          fatal("Unable to load FIDO enrollment "
                "challenge \"%s\": %s",
                opts[i] + 10, ssh_err(r));
        }
      } else if (strncasecmp(opts[i], "write-attestation=", 18) == 0) {
        sk_attestaion_path = opts[i] + 18;
      } else if (strncasecmp(opts[i], "application=", 12) == 0) {
    ...
    for (i = 0;; i++) {
      ...
      r = sshsk_enroll(type, sk_provider, sk_device,
                       ...
                       sk_user, sk_flags, passphrase, challenge, &private,
                       attest);
      if (r == 0)
        break;
      if (r != SSH_ERR_KEY_WRONG_PASSPHRASE)
  ...
  snprintf(dotsshdir, sizeof dotsshdir, "%s/%s", pw->pw_dir,
           _PATH_SSH_USER_DIR);
  if (strstr(identity_file, dotsshdir) != NULL) {
    if (stat(dotsshdir, &st) == -1) {
      if (errno != ENOENT) {
        ...
        error("Could not create directory '%s': %s", dotsshdir,
              strerror(errno));
      } else if (!quiet)
  ...
  if ((r = sshkey_save_private(private, identity_file, passphrase, comment,
                               private_key_format, openssh_format_cipher,
                               rounds)) != 0) {
    error("Saving key \"%s\" failed: %s", identity_file, ssh_err(r));
    freezero(passphrase, strlen(passphrase));
  ...
  if ((r = sshkey_save_public(public, identity_file, comment)) != 0) {
    fatal("Unable to save public key to %s: %s", identity_file,
          strerror(errno));
  }
