...
static u_int64_t parse_absolute_time(const char *s) {
  struct tm tm;
  time_t tt;
  char buf[32], *fmt;
  ...
  case 14:
    fmt = "%Y-%m-%dT%H:%M:%S";
    snprintf(buf, sizeof(buf), "%.4s-%.2s-%.2sT%.2s:%.2s:%.2s", s, s + 4, s + 6,
             s + 8, s + 10, s + 12);
    break;
  default:
...
static void update_krl_from_file(struct passwd *pw, const char *file,
  ...
  while (read_keyfile_line(krl_spec, path, line, sizeof(line), &lnum) == 0) {
