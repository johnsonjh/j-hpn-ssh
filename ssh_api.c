...
int _ssh_host_key_sign(struct sshkey *, struct sshkey *, u_char **, size_t *,
 ...
 */
int use_privsep = 0;
int mm_sshkey_sign(struct sshkey *, u_char **, u_int *, u_char *, u_int, char *,
                   u_int);
...
int mm_sshkey_sign(struct sshkey *key, u_char **sigp, u_int *lenp, u_char *data,
                   u_int datalen, char *alg, u_int compat) {
  return (-1);
}

DH *mm_choose_dh(int min, int nbits, int max) { return (NULL); }
...
void *ssh_get_app_data(struct ssh *ssh) { return ssh->app_data; }
  ...
  *typep = SSH_MSG_NONE;
  ...
  for (;;) {
    if ((r = ssh_packet_read_poll2(ssh, &type, &seqnr)) != 0)
      return r;
    if (type > 0 && type < DISPATCH_MAX && type >= SSH2_MSG_KEXINIT &&
        type <= SSH2_MSG_TRANSPORT_MAX && ssh->dispatch[type] != NULL) {
...
int ssh_packet_put(struct ssh *ssh, int type, const u_char *data, size_t len) {
  int r;

  if ((r = sshpkt_start(ssh, type)) != 0 ||
      (r = sshpkt_put(ssh, data, len)) != 0 || (r = sshpkt_send(ssh)) != 0)
    return r;
  return 0;
}
...
int _ssh_read_banner(struct ssh *ssh, char **bannerp) {
  struct sshbuf *input;
  const char *s;
  ...
  *bannerp = NULL;
  ...
  for (j = n = 0;;) {
    ...
    if (ssh->kex->server || ++n > 65536) {
      if ((r = sshbuf_put(ssh_packet_get_output(ssh), mismatch,
                          strlen(mismatch))) != 0)
        return r;
...
int _ssh_send_banner(struct ssh *ssh, char **bannerp) {
  char buf[256];
  int r;
...
int _ssh_exchange_banner(struct ssh *ssh) {
  ...
  if (kex->server) {
    ...
    if (r == 0 && kex->server_version_string != NULL &&
...
int _ssh_verify_host_key(struct sshkey *hostkey, struct ssh *ssh) {
  struct key_entry *k;
...
int _ssh_order_hostkeyalgs(struct ssh *ssh) {
  struct key_entry *k;
  ...
  *replace = '\0';
  while ((alg = strsep(&avail, ",")) && *alg != '\0') {
    if ((ktype = sshkey_type_from_name(alg)) == KEY_UNSPEC)
      continue;
    TAILQ_FOREACH(k, &ssh->public_keys, next) {
      if (k->key->type == ktype || (sshkey_is_cert(k->key) &&
...
int _ssh_host_key_sign(struct sshkey *privkey, struct sshkey *pubkey,
                       u_char **signature, size_t *slen, const u_char *data,
                       size_t dlen, const char *alg, u_int compat) {
