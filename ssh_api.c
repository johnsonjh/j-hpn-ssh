 ...
 */
int use_privsep = 0;
int mm_sshkey_sign(struct sshkey *, u_char **, u_int *, const u_char *, u_int,
...
int mm_sshkey_sign(struct sshkey *key, u_char **sigp, u_int *lenp,
...
#endif
#endif /* WITH_OPENSSL */
    ssh->kex->kex[KEX_KYBER_SHA256] = kex_gen_3way_server;
...
int _ssh_read_banner(struct ssh *ssh, struct sshbuf *banner) {
  ...
  for (j = n = 0;;) {
    ...
    if (sshbuf_len(banner) >= 4 && memcmp(sshbuf_ptr(banner), "SSH-", 4) == 0)
      break;
    debug("%s: %.*s", __func__, (int)sshbuf_len(banner), sshbuf_ptr(banner));
...
int _ssh_exchange_banner(struct ssh *ssh) {
  ...
  if (kex->server) {
    ...
    if (r == 0 && sshbuf_len(ssh->kex->server_version) != 0 &&
...
int _ssh_host_key_sign(struct ssh *ssh, struct sshkey *privkey,
                       struct sshkey *pubkey, u_char **signature, size_t *slen,
                       const u_char *data, size_t dlen, const char *alg) {
  return sshkey_sign(privkey, signature, slen, data, dlen, alg, NULL,
                     ssh->compat);
}
