...
struct sshsk_provider {
  ...
  int (*sk_sign)(int alg, const uint8_t *message, size_t message_len,
                 const char *application, const uint8_t *key_handle,
                 size_t key_handle_len, uint8_t flags, const char *pin,
                 struct sk_option **opts,
...
/* Built-in version */
int ssh_sk_enroll(int alg, const uint8_t *challenge, size_t challenge_len,
                  const char *application, uint8_t flags, const char *pin,
                  struct sk_option **opts,
                  struct sk_enroll_response **enroll_response);
...
static struct sshsk_provider *sshsk_open(const char *path) {
  ...
  if ((ret->sk_api_version = dlsym(ret->dlhandle, "sk_api_version")) == NULL) {
    error("Provider \"%s\" dlsym(sk_api_version) failed: %s", path, dlerror());
    goto fail;
  }
  version = ret->sk_api_version();
  debug("%s: provider %s implements version 0x%08lx", __func__, ret->path,
        (u_long)version);
  ...
  if ((ret->sk_enroll = dlsym(ret->dlhandle, "sk_enroll")) == NULL) {
    error("Provider %s dlsym(sk_enroll) failed: %s", path, dlerror());
    goto fail;
  }
  if ((ret->sk_sign = dlsym(ret->dlhandle, "sk_sign")) == NULL) {
    error("Provider \"%s\" dlsym(sk_sign) failed: %s", path, dlerror());
    goto fail;
  }
  if ((ret->sk_load_resident_keys =
           dlsym(ret->dlhandle, "sk_load_resident_keys")) == NULL) {
...
static void sshsk_free_enroll_response(struct sk_enroll_response *r) {
  if (r == NULL)
    return;
...
static int sshsk_ed25519_assemble(struct sk_enroll_response *resp,
                                  struct sshkey **keyp) {
  struct sshkey *key = NULL;
  int r;
...
static int sshsk_key_from_response(int alg, const char *application,
                                   uint8_t flags,
...
static int skerr_to_ssherr(int skerr) {
  switch (skerr) {
...
static void sshsk_free_options(struct sk_option **opts) {
  size_t i;
...
static int make_options(const char *device, const char *user_id,
...
int sshsk_enroll(int type, const char *provider_path, const char *device,
  ...
  debug("%s: provider \"%s\", device \"%s\", application \"%s\", "
        ...
        __func__, provider_path, device, application, userid, flags,
  ...
  *keyp = NULL;
  ...
  if (challenge_buf == NULL) {
    ...
    debug3("%s: using explicit challenge len=%zd", __func__, challenge_len);
  }
  ...
  /* XXX validate flags? */
  /* enroll key */
  if ((r = skp->sk_enroll(alg, challenge, challenge_len, application, flags,
                          pin, opts, &resp)) != 0) {
    debug("%s: provider \"%s\" returned failure %d", __func__, provider_path,
          r);
    r = skerr_to_ssherr(r);
    goto out;
  }

  if ((r = sshsk_key_from_response(alg, application, flags, resp, &key)) != 0)
    goto out;

  /* Optionally fill in the attestation information */
  if (attest != NULL) {
    if ((r = sshbuf_put_cstring(attest, "ssh-sk-attest-v00")) != 0 ||
        (r = sshbuf_put_string(attest, resp->attestation_cert,
                               resp->attestation_cert_len)) != 0 ||
        (r = sshbuf_put_string(attest, resp->signature, resp->signature_len)) !=
            0 ||
...
static int sshsk_ed25519_sig(struct sk_sign_response *resp,
                             struct sshbuf *sig) {
  int r = SSH_ERR_INTERNAL_ERROR;
  ...
  if ((r = sshbuf_put_string(sig, resp->sig_r, resp->sig_r_len)) != 0 ||
      (r = sshbuf_put_u8(sig, resp->flags)) != 0 ||
      (r = sshbuf_put_u32(sig, resp->counter)) != 0) {
    debug("%s: buffer error: %s", __func__, ssh_err(r));
    goto out;
...
int sshsk_sign(const char *provider_path, struct sshkey *key, u_char **sigp,
  ...
  debug("%s: provider \"%s\", key %s, flags 0x%02x%s", __func__, provider_path,
        sshkey_type(key), key->sk_flags,
        (pin != NULL && *pin != '\0') ? " with-pin" : "");
  ...
  if (provider_path == NULL || key->sk_key_handle == NULL ||
  ...
  if ((r = ssh_digest_memory(SSH_DIGEST_SHA256, data, datalen, message,
                             sizeof(message))) != 0) {
    error("%s: hash application failed: %s", __func__, ssh_err(r));
    r = SSH_ERR_INTERNAL_ERROR;
    goto out;
  }
  if ((r = skp->sk_sign(alg, message, sizeof(message), key->sk_application,
                        sshbuf_ptr(key->sk_key_handle),
                        sshbuf_len(key->sk_key_handle), key->sk_flags, pin,
                        opts, &resp)) != 0) {
...
static void sshsk_free_sk_resident_keys(struct sk_resident_key **rks,
                                        size_t nrks) {
  size_t i;
  ...
  for (i = 0; i < nrks; i++) {
    ...
    freezero(rks[i]->key.attestation_cert, rks[i]->key.attestation_cert_len);
...
int sshsk_load_resident(const char *provider_path, const char *device,
                        const char *pin, struct sshkey ***keysp,
                        size_t *nkeysp) {
  struct sshsk_provider *skp = NULL;
  int r = SSH_ERR_INTERNAL_ERROR;
  ...
  *nkeysp = 0;
  ...
  for (i = 0; i < nrks; i++) {
    debug3("%s: rk %zu: slot = %zu, alg = %d, application = \"%s\"", __func__,
    ...
    if ((r = sshsk_key_from_response(rks[i]->alg, rks[i]->application, flags,
                                     &rks[i]->key, &key)) != 0)
      goto out;
    if ((tmp = recallocarray(keys, nkeys, nkeys + 1, sizeof(*tmp))) == NULL) {
