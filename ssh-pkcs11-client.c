...
static void send_msg(struct sshbuf *m) {
  ...
  if (atomicio(vwrite, fd, buf, 4) != 4 ||
      atomicio(vwrite, fd, sshbuf_mutable_ptr(m), sshbuf_len(m)) !=
          sshbuf_len(m))
...
static int recv_msg(struct sshbuf *m) {
  u_int l, len;
  u_char c, buf[1024];
  int r;
...
int pkcs11_init(int interactive) { return (0); }
...
static int rsa_encrypt(int flen, const u_char *from, u_char *to, RSA *rsa,
                       int padding) {
...
static void wrap_key(struct sshkey *k) {
  if (k->type == KEY_RSA)
...
static int pkcs11_start_helper_methods(void) {
  if (helper_rsa != NULL)
    return (0);
...
static int pkcs11_start_helper(void) {
  int pair[2];
...
int pkcs11_add_provider(char *name, char *pin, struct sshkey ***keysp,
                        char ***labelsp) {
  struct sshkey *k;
  int r, type;
  u_char *blob;
...
int pkcs11_del_provider(char *name) {
  int r, ret = -1;
  struct sshbuf *msg;
