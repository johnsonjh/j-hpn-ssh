...
int sshsig_armor(const struct sshbuf *blob, struct sshbuf **out) {
  ...
  *out = NULL;
  ...
  if ((r = sshbuf_put(buf, BEGIN_SIGNATURE, sizeof(BEGIN_SIGNATURE) - 1)) !=
      0) {
  ...
  if ((r = sshbuf_dtob64(blob, buf, 1)) != 0) {
    error("%s: Couldn't base64 encode signature blob: %s", __func__,
          ssh_err(r));
    goto out;
  }

  if ((r = sshbuf_put(buf, END_SIGNATURE, sizeof(END_SIGNATURE) - 1)) != 0 ||
      (r = sshbuf_put_u8(buf, '\n')) != 0) {
...
int sshsig_dearmor(struct sshbuf *sig, struct sshbuf **out) {
  ...
  if ((r = sshbuf_cmp(sbuf, 0, BEGIN_SIGNATURE, sizeof(BEGIN_SIGNATURE) - 1)) !=
      0) {
...
static int sshsig_wrap_sign(struct sshkey *key, const char *hashalg,
  ...
  if ((tosign = sshbuf_new()) == NULL || (blob = sshbuf_new()) == NULL) {
    error("%s: sshbuf_new failed", __func__);
    r = SSH_ERR_ALLOC_FAIL;
    goto done;
  }
  ...
  if (signer != NULL) {
    if ((r = signer(key, &sig, &slen, sshbuf_ptr(tosign), sshbuf_len(tosign),
                    sign_alg, sk_provider, 0, signer_ctx)) != 0) {
      error("Couldn't sign message: %s", ssh_err(r));
      goto done;
    }
  } else {
    if ((r = sshkey_sign(key, &sig, &slen, sshbuf_ptr(tosign),
                         sshbuf_len(tosign), sign_alg, sk_provider, 0)) != 0) {
      error("Couldn't sign message: %s", ssh_err(r));
      goto done;
    }
  }
...
static int sshsig_parse_preamble(struct sshbuf *buf) {
  int r = SSH_ERR_INTERNAL_ERROR;
  uint32_t sversion;
  ...
  if (sversion > SIG_VERSION) {
    error("Signature version %lu is larger than supported "
          "version %u",
          (unsigned long)sversion, SIG_VERSION);
    return SSH_ERR_INVALID_FORMAT;
  }
  return 0;
}

static int sshsig_check_hashalg(const char *hashalg) {
  if (hashalg == NULL || match_pattern_list(hashalg, HASHALG_ALLOWED, 0) == 1)
    return 0;
...
static int sshsig_wrap_verify(struct sshbuf *signature, const char *hashalg,
  ...
  if ((r = sshbuf_put(toverify, MAGIC_PREAMBLE, MAGIC_PREAMBLE_LEN)) != 0 ||
  ...
  if (strcmp(expect_namespace, got_namespace) != 0) {
    ...
    debug("%s: expected namespace \"%s\" received \"%s\"", __func__,
  ...
  if (strcmp(hashalg, sig_hashalg) != 0) {
    ...
    debug("%s: expected algorithm \"%s\" received \"%s\"", __func__, hashalg,
          sig_hashalg);
    r = SSH_ERR_SIGNATURE_INVALID;
    goto done;
  }
...
static int hash_buffer(const struct sshbuf *m, const char *hashalg,
                       struct sshbuf **bp) {
  ...
  *bp = NULL;
  ...
  if ((alg = ssh_digest_alg_by_name(hashalg)) == -1) {
    error("%s: can't look up hash algorithm %s", __func__, hashalg);
...
int sshsig_signb(struct sshkey *key, const char *hashalg,
  ...
  if ((r = sshsig_wrap_sign(key, hashalg, sk_provider, b, sig_namespace, out,
                            signer, signer_ctx)) != 0)
    goto out;
  /* success */
  r = 0;
...
int sshsig_verifyb(struct sshbuf *signature, const struct sshbuf *message,
...
static int hash_file(int fd, const char *hashalg, struct sshbuf **bp) {
  ...
  *bp = NULL;
  ...
  for (;;) {
    ...
    if ((r = ssh_digest_update(ctx, rbuf, (size_t)n)) != 0) {
      error("%s: ssh_digest_update: %s", __func__, ssh_err(r));
      goto out;
    }
  }
...
int sshsig_sign_fd(struct sshkey *key, const char *hashalg,
                   ...
                   struct sshbuf **out, sshsig_signer *signer,
                   void *signer_ctx) {
  struct sshbuf *b = NULL;
  int r = SSH_ERR_INTERNAL_ERROR;
...
struct sshsigopt *sshsigopt_parse(const char *opts, const char *path,
                                  u_long linenum, const char **errstrp) {
...
static int parse_principals_key_and_options(const char *path, u_long linenum,
  ...
  if (required_principal != NULL) {
    ...
    debug("%s: %s:%lu: matched principal \"%s\"", __func__, path, linenum,
          required_principal);
  }
...
static int check_allowed_keys_line(const char *path, u_long linenum, char *line,
  ...
  /* Parse the line */
  if ((r = parse_principals_key_and_options(path, linenum, line, principal,
                                            NULL, &found_key, &sigopts)) != 0) {
  ...
  if (!sigopts->ca && sshkey_equal(found_key, sign_key)) {
    ...
    if ((r = sshkey_cert_check_authority(sign_key, 0, 1, principal, &reason)) !=
        0) {
      error("%s:%lu: certificate not authorized: %s", path, linenum, reason);
      goto done;
    }
...
int sshsig_check_allowed_keys(const char *path, const struct sshkey *sign_key,
  ...
  if ((f = fopen(path, "r")) == NULL) {
    oerrno = errno;
    error("Unable to open allowed keys file \"%s\": %s", path, strerror(errno));
    errno = oerrno;
...
static int cert_filter_principals(const char *path, u_long linenum,
  ...
  *principalsp = NULL;
  ...
  while ((cp = strsep(&principals, ",")) != NULL && *cp != '\0') {
    ...
    if ((r = sshkey_cert_check_authority(cert, 0, 1, cp, &reason)) != 0) {
      debug("%s:%lu: principal \"%s\" not authorized: %s", path, linenum, cp,
            reason);
      continue;
    }
...
static int get_matching_principals_from_line(const char *path, u_long linenum,
                                             char *line,
                                             const struct sshkey *sign_key,
                                             char **principalsp) {
  struct sshkey *found_key = NULL;
  ...
  /* Parse the line */
  if ((r = parse_principals_key_and_options(path, linenum, line, NULL,
                                            &principals, &found_key,
                                            &sigopts)) != 0) {
    /* error already logged */
    goto done;
  }
  ...
  if (!sigopts->ca && sshkey_equal(found_key, sign_key)) {
    ...
    if ((r = cert_filter_principals(path, linenum, &principals, sign_key)) !=
        0) {
      /* error already displayed */
      debug("%s:%lu: cert_filter_principals: %s", path, linenum, ssh_err(r));
      goto done;
    }
    debug("%s:%lu: matched certificate CA key", path, linenum);
    /* success */
...
int sshsig_find_principals(const char *path, const struct sshkey *sign_key,
  ...
  while (getline(&line, &linesize, f) != -1) {
    linenum++;
    r = get_matching_principals_from_line(path, linenum, line, sign_key,
                                          principals);
    free(line);
    line = NULL;
    if (r == SSH_ERR_KEY_NOT_FOUND)
  ...
  if (ferror(f) != 0) {
    oerrno = errno;
    fclose(f);
    error("Unable to read allowed keys file \"%s\": %s", path, strerror(errno));
    errno = oerrno;
    return SSH_ERR_SYSTEM_ERROR;
  }
  fclose(f);
