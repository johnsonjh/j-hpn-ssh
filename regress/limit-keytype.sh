...
# Return the required parameter for PubkeyAcceptedKeyAlgorithms corresponding to
...
prepare_config \
	"PubkeyAcceptedKeyAlgorithms $(keytype $ktype2),$(keytype $ktype1)"
...
prepare_config "PubkeyAcceptedKeyAlgorithms $(keytype $ktype1)"
${SSH} $certopts proxy true && fatal "cert succeeded"
${SSH} $opts -i $OBJ/user_key1 proxy true || fatal "key1 failed"
...
prepare_config "PubkeyAcceptedKeyAlgorithms *-cert-v01@openssh.com"
${SSH} $certopts proxy true || fatal "cert failed"
...
prepare_config "PubkeyAcceptedKeyAlgorithms $(keytype $ktype2)" \
	"Match user x$USER" "PubkeyAcceptedKeyAlgorithms +$(keytype $ktype1)"
${SSH} $certopts proxy true && fatal "cert succeeded"
if [ "$ktype1" != "$ktype2" ]; then
	${SSH} $opts -i $OBJ/user_key1 proxy true && fatal "key1 succeeded"
fi
...
prepare_config "PubkeyAcceptedKeyAlgorithms $(keytype $ktype4)" \
	"Match user $USER" "PubkeyAcceptedKeyAlgorithms +$(keytype $ktype1)"
${SSH} $certopts proxy true || fatal "cert failed"
${SSH} $opts -i $OBJ/user_key1 proxy true || fatal "key1 failed"
