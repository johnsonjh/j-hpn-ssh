...
static int do_send_and_receive(struct ssh *from, struct ssh *to) {
  u_char type;
  size_t len;
  const u_char *buf;
  int r;
...
static void run_kex(struct ssh *client, struct ssh *server) {
  int r = 0;
...
static void do_kex_with_key(char *kex, int keytype, int bits) {
...
#endif /* HAVE_SIKE */
#ifdef HAVE_HQC
  do_kex(KEX_ECDH_NISTP384_HQC_128_SHA384);
  do_kex(KEX_ECDH_NISTP384_HQC_192_SHA384);
  do_kex(KEX_ECDH_NISTP384_HQC_256_SHA384);
#endif /* HAVE_HQC */
#ifdef HAVE_NTRU_PRIME
  do_kex(KEX_ECDH_NISTP384_NTRULPR_653_SHA384);
  do_kex(KEX_ECDH_NISTP384_NTRULPR_761_SHA384);
  do_kex(KEX_ECDH_NISTP384_NTRULPR_857_SHA384);
  do_kex(KEX_ECDH_NISTP384_SNTRUP_653_SHA384);
  do_kex(KEX_ECDH_NISTP384_SNTRUP_761_SHA384);
  do_kex(KEX_ECDH_NISTP384_SNTRUP_857_SHA384);
...
#endif /* HAVE_SIKE */
#ifdef HAVE_HQC
  do_kex(KEX_HQC_128_SHA384);
  do_kex(KEX_HQC_192_SHA384);
  do_kex(KEX_HQC_256_SHA384);
#endif /* HAVE_HQC */
#ifdef HAVE_NTRU_PRIME
  do_kex(KEX_NTRULPR_653_SHA384);
  do_kex(KEX_NTRULPR_761_SHA384);
  do_kex(KEX_NTRULPR_857_SHA384);
  do_kex(KEX_SNTRUP_653_SHA384);
  do_kex(KEX_SNTRUP_761_SHA384);
  do_kex(KEX_SNTRUP_857_SHA384);
#endif /* HAVE_NTRU_PRIME */
