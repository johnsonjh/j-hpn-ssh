...
static void put_opt(struct sshbuf *b, const char *name, const char *value) {
  struct sshbuf *sect;
...
#ifdef WITH_OPENSSL
static void build_cert(struct sshbuf *b, struct sshkey *k, const char *type,
  ...
  ASSERT_INT_EQ(sshkey_sign(sign_key, &sigblob, &siglen, sshbuf_ptr(b),
                            sshbuf_len(b), sig_alg, NULL, 0),
                0);
...
static void signature_test(struct sshkey *k, struct sshkey *bad,
                           const char *sig_alg, const u_char *d, size_t l) {
  size_t len;
  u_char *sig;
...
static void banana(u_char *s, size_t l) {
  size_t o;
...
static void signature_tests(struct sshkey *k, struct sshkey *bad,
                            const char *sig_alg) {
  u_char i, buf[2049];
  size_t lens[] = {1,   2,    7,    8,    9,    15,   16,  17,
                   31,  32,   33,   127,  128,  129,  255, 256,
                   257, 1023, 1024, 1025, 2047, 2048, 2049};

  for (i = 0; i < (sizeof(lens) / sizeof(lens[0])); i++) {
    test_subtest_info("%s key, banana length %zu", sshkey_type(k), lens[i]);
    banana(buf, lens[i]);
    signature_test(k, bad, sig_alg, buf, lens[i]);
  }
}

static struct sshkey *get_private(const char *n) {
  struct sshbuf *b;
  struct sshkey *ret;
...
void sshkey_tests(void) {
  ...
  ASSERT_INT_EQ(sshkey_generate(KEY_RSA, 128, &k1), SSH_ERR_KEY_LENGTH);
  ASSERT_PTR_EQ(k1, NULL);
  TEST_DONE();

  TEST_START("generate KEY_RSA too large modulus");
  ASSERT_INT_EQ(sshkey_generate(KEY_RSA, 1 << 20, &k1), SSH_ERR_KEY_LENGTH);
  ASSERT_PTR_EQ(k1, NULL);
  TEST_DONE();

  TEST_START("generate KEY_DSA wrong bits");
  ASSERT_INT_EQ(sshkey_generate(KEY_DSA, 2048, &k1), SSH_ERR_KEY_LENGTH);
  ASSERT_PTR_EQ(k1, NULL);
  sshkey_free(k1);
  TEST_DONE();
  ...
  ASSERT_INT_EQ(sshkey_generate(KEY_ECDSA, 42, &k1), SSH_ERR_KEY_LENGTH);
  ASSERT_PTR_EQ(k1, NULL);
  sshkey_free(k1);
  TEST_DONE();
#endif
  ...
  ASSERT_INT_EQ(sshkey_generate(KEY_RSA, 767, &kr), SSH_ERR_KEY_LENGTH);
  ...
  ASSERT_INT_EQ(sshkey_load_public(test_data_file("ed25519_1.pub"), &k1, NULL),
                0);
  ...
  ASSERT_INT_EQ(sshkey_load_public(test_data_file("rsa_2.pub"), &k2, NULL), 0);
  ...
  ASSERT_INT_EQ(sshkey_load_public(test_data_file("dsa_2.pub"), &k2, NULL), 0);
  ...
  ASSERT_INT_EQ(sshkey_load_public(test_data_file("ecdsa_2.pub"), &k2, NULL),
                0);
  signature_tests(k1, k2, NULL);
  sshkey_free(k1);
  sshkey_free(k2);
  ...
  ASSERT_INT_EQ(sshkey_load_public(test_data_file("ed25519_2.pub"), &k2, NULL),
                0);
  signature_tests(k1, k2, NULL);
  sshkey_free(k1);
  sshkey_free(k2);
  ...
  ASSERT_INT_EQ(sshkey_load_public(test_data_file("rsa_1.pub"), &k2, NULL), 0);
