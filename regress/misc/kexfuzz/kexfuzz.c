...
static int packet_hook(struct ssh *ssh, struct sshbuf *packet, u_char *typep,
                       void *_ctx) {
  ...
  if (mydirection == ctx->trigger_direction &&
    ...
    if (ctx->replace_data != NULL) {
      ...
      if ((r = sshbuf_get_u8(ctx->replace_data, typep)) != 0 ||
      ...
      if (sshbuf_len(packet) != 0 &&
          fwrite(sshbuf_ptr(packet), sshbuf_len(packet), 1, dumpfile) != 1)
  ...
  do {                                                                         \
    if ((int)(a) != (int)(b)) {                                                \
      fprintf(stderr,                                                          \
              "%s %s:%d "                                                      \
              "%s (%d) != expected %s (%d)\n",                                 \
              in_test ? in_test : "(none)", __func__, __LINE__, #a, (int)(a),  \
              #b, (int)(b));                                                   \
      exit(2);                                                                 \
    }                                                                          \
  } while (0)
  ...
  do {                                                                         \
    if ((a) == (b)) {                                                          \
      fprintf(stderr,                                                          \
              "%s %s:%d "                                                      \
              "%s (%p) != expected %s (%p)\n",                                 \
              in_test ? in_test : "(none)", __func__, __LINE__, #a, (a), #b,   \
              (b));                                                            \
      exit(2);                                                                 \
    }                                                                          \
  } while (0)
...
static void do_kex_with_key(const char *kex, struct sshkey *prvkey, int *c2s,
                            int *s2c, int direction, int packet_index,
                            const char *dump_path,
                            struct sshbuf *replace_data) {
...
static void badusage(const char *bad) {
...
int main(int argc, char **argv) {
  ...
  if ((fd = open(kpath, O_RDONLY)) == -1)
    err(1, "open %s", kpath);
  if ((r = sshkey_load_private_type_fd(fd, KEY_UNSPEC, NULL, &key, NULL)) != 0)
  ...
  do_kex_with_key(kex, key, &c2s, &s2c, direction, packet_index,
