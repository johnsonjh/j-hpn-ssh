...
int sshkey_oqs_generate_private_key(struct sshkey *k, int type) {
...
int ssh_oqs_sign(const struct sshkey *key, u_char **sigp, size_t *lenp,
  ...
  if (key == NULL || !IS_OQS_KEY_TYPE(sshkey_type_plain(key->type)) ||
      key->oqs_sk == NULL)
    return SSH_ERR_INVALID_ARGUMENT;
  siglen = key->oqs_sig->length_signature;
  if ((sig = malloc(siglen)) == NULL)
    return SSH_ERR_ALLOC_FAIL;
  if (OQS_SIG_sign(key->oqs_sig, sig, &siglen, data, datalen, key->oqs_sk) !=
      OQS_SUCCESS) {
  ...
  /* OQS note: all the OQS algs use the same format, so we identify the
   * signature as "ssh-oqs" */
...
int ssh_oqs_verify(const struct sshkey *key, const u_char *signature,
  ...
  if (key == NULL || !IS_OQS_KEY_TYPE(sshkey_type_plain(key->type)) ||
      key->oqs_pk == NULL || signature == NULL || signaturelen == 0)
    return SSH_ERR_INVALID_ARGUMENT;
  ...
  if (OQS_SIG_verify(key->oqs_sig, data, datalen, sigblob, slen, key->oqs_pk) !=
      OQS_SUCCESS) {
