...
static char *expand_proxy_command(const char *proxy_command, const char *user,
                                  const char *host, int port) {
  char *tmp, *ret, strport[NI_MAXSERV];

  snprintf(strport, sizeof strport, "%d", port);
  xasprintf(&tmp, "exec %s", proxy_command);
  ret = percent_expand(tmp, "h", host, "p", strport, "r", options.user,
                       (char *)NULL);
  free(tmp);
  return ret;
}
 ...
 */
static int ssh_proxy_fdpass_connect(struct ssh *ssh, const char *host,
  ...
  command_string =
      expand_proxy_command(proxy_command, options.user, host, port);
 ...
 */
static int ssh_proxy_connect(struct ssh *ssh, const char *host, u_short port,
 ...
 */
static int check_ifaddrs(const char *ifname, int af,
  ...
  for (allow_local = 0; allow_local < 2; allow_local++) {
    for (ifa = ifaddrs; ifa != NULL; ifa = ifa->ifa_next) {
      if (ifa->ifa_addr == NULL || ifa->ifa_name == NULL ||
          (ifa->ifa_flags & IFF_UP) == 0 || ifa->ifa_addr->sa_family != af ||
          strcmp(ifa->ifa_name, options.bind_interface) != 0)
        continue;
      switch (ifa->ifa_addr->sa_family) {
      case AF_INET:
        sa = (struct sockaddr_in *)ifa->ifa_addr;
        if (!allow_local && sa->sin_addr.s_addr == htonl(INADDR_LOOPBACK))
          continue;
        if (*rlenp < sizeof(struct sockaddr_in)) {
          error("%s: v4 addr doesn't fit", __func__);
          return -1;
        }
      ...
      case AF_INET6:
        ...
        if (!allow_local &&
            (IN6_IS_ADDR_LINKLOCAL(v6addr) || IN6_IS_ADDR_LOOPBACK(v6addr)))
          continue;
        if (*rlenp < sizeof(struct sockaddr_in6)) {
          error("%s: v6 addr doesn't fit", __func__);
          return -1;
        }
 ...
 */
static int ssh_create_socket(struct addrinfo *ai) {
  int sock, r;
  ...
  if (options.bind_address != NULL) {
    ...
    if ((r = getaddrinfo(options.bind_address, NULL, &hints, &res)) != 0) {
      error("getaddrinfo: %s: %s", options.bind_address, ssh_gai_strerror(r));
      goto fail;
    }
    if (res == NULL) {
    ...
    if ((r = getifaddrs(&ifaddrs)) != 0) {
      error("getifaddrs: %s: %s", options.bind_interface, strerror(errno));
      goto fail;
    }
    bindaddrlen = sizeof(bindaddr);
    if (check_ifaddrs(options.bind_interface, ai->ai_family, ifaddrs, &bindaddr,
                      &bindaddrlen) != 0) {
      logit("getifaddrs: %s: no suitable addresses", options.bind_interface);
      goto fail;
    }
#else
  ...
  if ((r = getnameinfo((struct sockaddr *)&bindaddr, bindaddrlen, ntop,
                       sizeof(ntop), NULL, 0, NI_NUMERICHOST)) != 0) {
    error("%s: getnameinfo failed: %s", __func__, ssh_gai_strerror(r));
    goto fail;
  }
...
static int timeout_connect(int sockfd, const struct sockaddr *serv_addr,
...
static int ssh_connect_direct(struct ssh *ssh, const char *host,
                              ...
                              int family, int connection_attempts,
  ...
  for (attempt = 0; attempt < connection_attempts; attempt++) {
    ...
    for (ai = aitop; ai; ai = ai->ai_next) {
      ...
      if (timeout_connect(sock, ai->ai_addr, ai->ai_addrlen, timeout_ms) >= 0) {
        ...
        oerrno = errno;
        debug("connect to address %s port %s: %s", ntop, strport,
              strerror(errno));
        close(sock);
        sock = -1;
        errno = oerrno;
...
int ssh_connect(struct ssh *ssh, const char *host, struct addrinfo *addrs,
                struct sockaddr_storage *hostaddr, u_short port, int family,
                int connection_attempts, int *timeout_ms, int want_keepalive) {
  if (options.proxy_command == NULL) {
    return ssh_connect_direct(ssh, host, addrs, hostaddr, port, family,
                              connection_attempts, timeout_ms, want_keepalive);
  } else if (strcmp(options.proxy_command, "-") == 0) {
    if ((ssh_packet_set_connection(ssh, STDIN_FILENO, STDOUT_FILENO)) == NULL)
      return -1; /* ssh_packet_set_connection logs error */
    return 0;
  } else if (options.proxy_use_fdpass) {
    return ssh_proxy_fdpass_connect(ssh, host, port, options.proxy_command);
  }
  return ssh_proxy_connect(ssh, host, port, options.proxy_command);
}

static void send_client_banner(int connection_out, int minor1) {
  /* Send our own protocol version identification. */
  xasprintf(&client_version_string, "SSH-%d.%d-%.100s\r\n", PROTOCOL_MAJOR_2,
            PROTOCOL_MINOR_2, SSH_VERSION);
 ...
 */
void ssh_exchange_identification(int timeout_ms) {
  ...
  if (mismatch)
    fatal("Protocol major versions differ: %d vs. %d", PROTOCOL_MAJOR_2,
          remote_major);
...
static int check_host_cert(const char *host, const struct sshkey *key) {
  const char *reason;
  int r;
...
static int sockaddr_is_local(struct sockaddr *hostaddr) {
  switch (hostaddr->sa_family) {
  case AF_INET:
    return (ntohl(((struct sockaddr_in *)hostaddr)->sin_addr.s_addr) >> 24) ==
           IN_LOOPBACKNET;
  case AF_INET6:
 ...
 */
void get_hostfile_hostname_ipaddr(char *hostname, struct sockaddr *hostaddr,
                                  u_short port, char **hostfile_hostname,
  ...
  if (hostfile_ipaddr != NULL) {
    if (options.proxy_command == NULL) {
      if (getnameinfo(hostaddr, addrlen, ntop, sizeof(ntop), NULL, 0,
                      NI_NUMERICHOST) != 0)
...
#define RDONLY 1
#define ROQUIET 2
static int check_host_key(char *hostname, struct sockaddr *hostaddr,
                          u_short port, struct sshkey *host_key, int readonly,
  ...
  if (options.check_host_ip &&
      (local || strcmp(hostname, ip) == 0 || options.proxy_command != NULL))
...
retry:
  ...
  if (!want_cert && ip_hostkeys != NULL) {
    ...
    if (host_status == HOST_CHANGED &&
        (ip_status != HOST_CHANGED ||
         (ip_found != NULL && !sshkey_equal(ip_found->key, host_found->key))))
      host_ip_differ = 1;
  } else
  ...
  case HOST_OK:
    ...
    debug("Host '%.200s' is known and matches the %s host %s.", host, type,
          want_cert ? "certificate" : "key");
    debug("Found %s in %s:%lu", want_cert ? "CA key" : "key", host_found->file,
          host_found->line);
    if (want_cert && !check_host_cert(options.host_key_alias == NULL
                                          ? hostname
  ...
  case HOST_NEW:
    if (options.host_key_alias == NULL && port != 0 &&
      ...
      if (check_host_key(hostname, hostaddr, 0, host_key, ROQUIET,
    ...
    if (readonly || want_cert)
      goto fail;
    /* The host is new. */
    if (options.strict_host_key_checking == SSH_STRICT_HOSTKEY_YES) {
      /*
      ...
      snprintf(msg, sizeof(msg),
               ...
               "(yes/no)? ",
               host, ip, msg1, type, fp, options.visual_host_key ? "\n" : "",
               options.visual_host_key ? ra : "", msg2);
      free(ra);
      free(fp);
      if (!confirm(msg))
        goto fail;
  ...
  case HOST_CHANGED:
    if (want_cert) {
      ...
      debug("Host certificate authority does not "
            "match %s in %s:%lu",
            CA_MARKER, host_found->file, host_found->line);
      goto fail;
    }
    if (readonly == ROQUIET)
      goto fail;
    if (options.check_host_ip && host_ip_differ) {
      ...
      if (ip_status != HOST_NEW)
        error("Offending key for IP in %s:%lu", ip_found->file, ip_found->line);
    }
    /* The host key has changed. */
    warn_changed_key(host_key);
    error("Add correct host key in %.100s to get rid of this message.",
          user_hostfiles[0]);
    error("Offending %s key in %s:%lu", sshkey_type(host_found->key),
  ...
  if (options.check_host_ip && host_status != HOST_CHANGED &&
    ...
    if (options.strict_host_key_checking == SSH_STRICT_HOSTKEY_ASK) {
      strlcat(msg,
      ...
      if (!confirm(msg))
        goto fail;
    } else if (options.strict_host_key_checking != SSH_STRICT_HOSTKEY_OFF) {
      logit("%s", msg);
...
fail:
  if (want_cert && host_status != HOST_REVOKED) {
    ...
    if ((r = sshkey_from_private(host_key, &raw_key)) != 0)
      fatal("%s: sshkey_from_private: %s", __func__, ssh_err(r));
...
int verify_host_key(char *host, struct sockaddr *hostaddr,
  ...
  if ((fp = sshkey_fingerprint(host_key, options.fingerprint_hash,
                               SSH_FP_DEFAULT)) == NULL) {
  ...
  if (sshkey_is_cert(host_key)) {
    if ((cafp = sshkey_fingerprint(host_key->cert->signature_key,
        ...
        NULL) {
      error("%s: fingerprint CA key: %s", __func__, ssh_err(r));
      r = -1;
      goto out;
    }
    sshkey_format_cert_validity(host_key->cert, valid, sizeof(valid));
    debug("Server host certificate: %s %s, serial %llu "
          "ID \"%s\" CA %s %s valid %s",
          sshkey_ssh_name(host_key), fp,
          (unsigned long long)host_key->cert->serial, host_key->cert->key_id,
          sshkey_ssh_name(host_key->cert->signature_key), cafp, valid);
  ...
  if (sshkey_equal(previous_host_key, host_key)) {
    debug2("%s: server host key %s %s matches cached key", __func__,
  ...
  if (options.revoked_host_keys != NULL) {
    ...
    case SSH_ERR_KEY_REVOKED:
      error("Host key %s %s revoked by file %s", sshkey_type(host_key), fp,
    ...
    default:
      error("Error checking host key %s %s in "
            ...
            sshkey_type(host_key), fp, options.revoked_host_keys, ssh_err(r));
      r = -1;
      goto out;
    }
  }
  ...
  if (options.verify_host_key_dns) {
    ...
    if (verify_host_key_dns(host, hostaddr, plain, &flags) == 0) {
      if (flags & DNS_VERIFY_FOUND) {
        if (options.verify_host_key_dns == 1 && flags & DNS_VERIFY_MATCH &&
 ...
 */
void ssh_login(Sensitive *sensitive, const char *orighost,
               struct sockaddr *hostaddr, u_short port, struct passwd *pw,
               int timeout_ms) {
  char *host;
...
static int show_other_keys(struct hostkeys *hostkeys, struct sshkey *key) {
  int type[] = {
      KEY_RSA, KEY_DSA, KEY_ECDSA, KEY_ED25519, KEY_XMSS,
      ///// OQS_TEMPLATE_FRAGMENT_LIST_KT_START
      KEY_OQSDEFAULT, KEY_DILITHIUM_2, KEY_FALCON_512, KEY_PICNIC_L1FULL,
      KEY_PICNIC3_L1, KEY_RAINBOW_I_CLASSIC, KEY_RAINBOW_III_CLASSIC,
      KEY_RAINBOW_V_CLASSIC, KEY_SPHINCS_HARAKA_128F_ROBUST,
      KEY_SPHINCS_SHA256_128F_ROBUST, KEY_SPHINCS_SHAKE256_128F_ROBUST,
      KEY_RSA3072_OQSDEFAULT, KEY_P256_OQSDEFAULT, KEY_RSA3072_DILITHIUM_2,
      KEY_P256_DILITHIUM_2, KEY_RSA3072_FALCON_512, KEY_P256_FALCON_512,
      KEY_RSA3072_PICNIC_L1FULL, KEY_P256_PICNIC_L1FULL, KEY_RSA3072_PICNIC3_L1,
      KEY_P256_PICNIC3_L1, KEY_RSA3072_RAINBOW_I_CLASSIC,
      KEY_P256_RAINBOW_I_CLASSIC, KEY_P384_RAINBOW_III_CLASSIC,
      KEY_P521_RAINBOW_V_CLASSIC, KEY_RSA3072_SPHINCS_HARAKA_128F_ROBUST,
  ...
  for (i = 0; type[i] != -1; i++) {
    ...
    if (!lookup_key_in_hostkeys_by_type(hostkeys, type[i], &found))
      continue;
    fp = sshkey_fingerprint(found->key, options.fingerprint_hash,
                            SSH_FP_DEFAULT);
    ra = sshkey_fingerprint(found->key, options.fingerprint_hash,
                            SSH_FP_RANDOMART);
    if (fp == NULL || ra == NULL)
      fatal("%s: sshkey_fingerprint fail", __func__);
    logit("WARNING: %s key found for host %s\n"
          "in %s:%lu\n"
          "%s key fingerprint %s.",
          sshkey_type(found->key), found->host, found->file, found->line,
...
static void warn_changed_key(struct sshkey *host_key) {
  ...
  error("Someone could be eavesdropping on you right now (man-in-the-middle "
        "attack)!");
...
int ssh_local_cmd(const char *args) {
  ...
  if (!options.permit_local_command || args == NULL || !*args)
    return (1);
  ...
  if (pid == 0) {
    ...
    error("Couldn't execute %s -c \"%s\": %s", shell, args, strerror(errno));
    _exit(1);
  } else if (pid == -1)
    fatal("fork failed: %.100s", strerror(errno));
...
void maybe_add_key_to_agent(char *authfile, const struct sshkey *private,
