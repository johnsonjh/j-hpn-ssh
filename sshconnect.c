...
static void stderr_null(void) {
  int devnull;

  if ((devnull = open(_PATH_DEVNULL, O_WRONLY)) == -1) {
    error("Can't open %s for stderr redirection: %s", _PATH_DEVNULL,
          strerror(errno));
    return;
  }
...
static int ssh_proxy_fdpass_connect(struct ssh *ssh, const char *host,
  ...
  if ((pid = fork()) == 0) {
     ...
     */
    if (!debug_flag && options.control_path != NULL && options.control_persist)
      stderr_null();
...
static int ssh_create_socket(struct addrinfo *ai) {
  ...
  if (options.bind_address != NULL || source_port != NULL) {
    ...
    if ((r = getaddrinfo(options.bind_address, source_port, &hints, &res)) !=
        0) {
  ...
  for (msg = prompt;; msg = fingerprint ? again_fp : again) {
    ...
    if (p[0] == '\0' || strcasecmp(p, "no") == 0)
      ret = 0;
             ...
             (fingerprint != NULL && strcasecmp(p, fingerprint) == 0))
      ret = 1;
    free(p);
    if (ret != -1)
      return ret;
...
static int show_other_keys(struct hostkeys *hostkeys, struct sshkey *key) {
  int type[] = {KEY_RSA, KEY_DSA, KEY_ECDSA, KEY_ED25519, KEY_XMSS, -1};
  int i, ret = 0;
  char *fp, *ra;
