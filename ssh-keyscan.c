...
#define KT_RSA3072_OQSDEFAULT ((uint64_t)1 << 16)
#define KT_P256_OQSDEFAULT ((uint64_t)1 << 17)
#define KT_RSA3072_DILITHIUM_2 ((uint64_t)1 << 18)
...
#define KT_P256_SPHINCS_SHA256_128F_ROBUST ((uint64_t)1 << 33)
#define KT_RSA3072_SPHINCS_SHAKE256_128F_ROBUST ((uint64_t)1 << 34)
#define KT_P256_SPHINCS_SHAKE256_128F_ROBUST ((uint64_t)1 << 35)
...
int get_cert = 0;
uint64_t get_keytypes =
    KT_RSA | KT_ECDSA | KT_ED25519 | ///// OQS_TEMPLATE_FRAGMENT_GET_KT_START
    KT_OQSDEFAULT | KT_DILITHIUM_2 | KT_FALCON_512 | KT_PICNIC_L1FULL |
    KT_PICNIC3_L1 | KT_RAINBOW_I_CLASSIC | KT_RAINBOW_III_CLASSIC |
    KT_RAINBOW_V_CLASSIC | KT_SPHINCS_HARAKA_128F_ROBUST |
    KT_SPHINCS_SHA256_128F_ROBUST | KT_SPHINCS_SHAKE256_128F_ROBUST |
    KT_RSA3072_OQSDEFAULT | KT_P256_OQSDEFAULT | KT_RSA3072_DILITHIUM_2 |
    KT_P256_DILITHIUM_2 | KT_RSA3072_FALCON_512 | KT_P256_FALCON_512 |
    KT_RSA3072_PICNIC_L1FULL | KT_P256_PICNIC_L1FULL | KT_RSA3072_PICNIC3_L1 |
    KT_P256_PICNIC3_L1 | KT_RSA3072_RAINBOW_I_CLASSIC |
    KT_P256_RAINBOW_I_CLASSIC | KT_P384_RAINBOW_III_CLASSIC |
    KT_P521_RAINBOW_V_CLASSIC | KT_RSA3072_SPHINCS_HARAKA_128F_ROBUST |
    KT_P256_SPHINCS_HARAKA_128F_ROBUST | KT_RSA3072_SPHINCS_SHA256_128F_ROBUST |
 ...
 * one in the GNU libc.
 */
static char *xstrsep(char **str, const char *delim) {
  char *s, *e;
 ...
 */
static char *strnnsep(char **stringp, char *delim) {
  char *tok;
...
static int key_print_wrapper(struct sshkey *hostkey, struct ssh *ssh) {
  con *c;
...
static void keygrab_ssh2(con *c) {
  ...
  case KT_DSA:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "ssh-dss-cert-v01@openssh.com" : "ssh-dss";
    break;
  case KT_RSA:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "ssh-rsa-cert-v01@openssh.com" : "ssh-rsa";
    break;
  case KT_ED25519:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "ssh-ed25519-cert-v01@openssh.com" : "ssh-ed25519";
    break;
  case KT_XMSS:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "ssh-xmss-cert-v01@openssh.com" : "ssh-xmss@openssh.com";
    break;
  case KT_ECDSA:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "ecdsa-sha2-nistp256-cert-v01@openssh.com,"
...
static void keyprint_one(const char *host, struct sshkey *key) {
  char *hostport;
...
static int tcpconnect(char *host) {
  ...
  for (ai = aitop; ai; ai = ai->ai_next) {
    ...
    if (connect(s, ai->ai_addr, ai->ai_addrlen) < 0 && errno != EINPROGRESS)
...
static int conalloc(char *iname, char *oname, int keytype) {
