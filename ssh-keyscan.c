...
static void keygrab_ssh2(con *c) {
  ...
  case KT_RSA:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "rsa-sha2-512-cert-v01@openssh.com,"
  ...
  case KT_ECDSA_SK:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "sk-ecdsa-sha2-nistp256-cert-v01@openssh.com"
                 : "sk-ecdsa-sha2-nistp256@openssh.com";
    break;
  case KT_ED25519_SK:
    myproposal[PROPOSAL_SERVER_HOST_KEY_ALGS] =
        get_cert ? "sk-ssh-ed25519-cert-v01@openssh.com"
...
#endif
#endif
  c->c_ssh->kex->kex[KEX_KYBER_SHA256] = kex_gen_3way_client;
...
static int tcpconnect(char *host) {
  ...
  for (ai = aitop; ai; ai = ai->ai_next) {
    ...
    if (connect(s, ai->ai_addr, ai->ai_addrlen) == -1 && errno != EINPROGRESS)
