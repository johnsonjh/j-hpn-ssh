...
static void add_listen_addr(ServerOptions *, const char *, const char *, int);
static void add_one_listen_addr(ServerOptions *, const char *, const char *,
                                int);
...
static int option_clear_or_none(const char *o) {
...
static void array_append(const char *file, const int line,
                         const char *directive, char ***array, u_int *lp,
                         const char *s) {
...
void servconf_add_hostkey(const char *file, const int line,
                          ServerOptions *options, const char *path) {
  char *apath = derelativise_path(path);

  array_append(file, line, "HostKey", &options->host_key_files,
               &options->num_host_key_files, apath);
  free(apath);
}

void servconf_add_hostcert(const char *file, const int line,
                           ServerOptions *options, const char *path) {
  char *apath = derelativise_path(path);

  array_append(file, line, "HostCertificate", &options->host_cert_files,
               &options->num_host_cert_files, apath);
  free(apath);
}
...
void fill_default_server_options(ServerOptions *options) {
  ...
  if (options->num_host_key_files == 0) {
    ...
    servconf_add_hostkey("[default]", 0, options, _PATH_HOST_RSA_KEY_FILE);
#ifdef OPENSSL_HAS_ECC
    servconf_add_hostkey("[default]", 0, options, _PATH_HOST_ECDSA_KEY_FILE);
#endif
    servconf_add_hostkey("[default]", 0, options, _PATH_HOST_ED25519_KEY_FILE);
#ifdef WITH_XMSS
    servconf_add_hostkey("[default]", 0, options, _PATH_HOST_XMSS_KEY_FILE);
#endif /* WITH_XMSS */
#ifdef WITH_PQ_AUTH
...
/* Portable-specific options */
    ...
    {"authorizedprincipalscommanduser", sAuthorizedPrincipalsCommandUser,
     SSHCFG_ALL},
...
static struct {
  int val;
  char *text;
} tunmode_desc[] = {{SSH_TUNMODE_NO, "no"},
...
char *derelativise_path(const char *path) {
...
static void add_listen_addr(ServerOptions *options, const char *addr,
  ...
  if (port > 0)
    ...
    for (i = 0; i < options->num_ports; i++) {
      add_one_listen_addr(options, addr, rdomain, options->ports[i]);
    }
  }
}

static void add_one_listen_addr(ServerOptions *options, const char *addr,
                                const char *rdomain, int port) {
  ...
  if (i >= options->num_listen_addrs) {
    ...
    options->listen_addrs = xrecallocarray(
        options->listen_addrs, options->num_listen_addrs,
        options->num_listen_addrs + 1, sizeof(*options->listen_addrs));
  ...
  if ((gaierr = getaddrinfo(addr, strport, &hints, &aitop)) != 0)
    fatal("bad addr or host: %s (%s)", addr ? addr : "<NULL>",
...
static int valid_rdomain(const char *name) {
 ...
 */
static void queue_listen_addr(ServerOptions *options, const char *addr,
                              const char *rdomain, int port) {
  struct queued_listenaddr *qla;

  options->queued_listen_addrs = xrecallocarray(
      options->queued_listen_addrs, options->num_queued_listens,
      options->num_queued_listens + 1, sizeof(*options->queued_listen_addrs));
 ...
 */
static void process_permitopen_list(struct ssh *ssh, ServerOpCodes opcode,
  ...
  for (i = 0; i < num_opens; i++) {
    ...
    channel_add_permission(ssh, FORWARD_ADM, where, host, port);
    free(oarg);
  }
}
...
void process_permitopen(struct ssh *ssh, ServerOptions *options) {
  process_permitopen_list(ssh, sPermitOpen, options->permitted_opens,
                          options->num_permitted_opens);
  process_permitopen_list(ssh, sPermitListen, options->permitted_listens,
...
static int match_cfg_line_group(const char *grps, int line, const char *user) {
  int result = 0;
  struct passwd *pw;
...
static void match_test_missing_fatal(const char *criteria, const char *attrib) {
  ...
  while ((attrib = strdelim(&cp)) && *attrib != '\0') {
    ...
    if (strcasecmp(attrib, "user") == 0) {
      ...
      if ((port = a2port(arg)) == -1) {
        error("Invalid LocalPort '%s' on Match line", arg);
        return -1;
      }
      if (ci == NULL) {
        result = 0;
...
static const struct multistate multistate_gatewayports[] = {
    {"clientspecified", 2}, {"yes", 1}, {"no", 0}, {NULL, -1}};
static const struct multistate multistate_tcpfwd[] = {
    {"yes", FORWARD_ALLOW},     {"all", FORWARD_ALLOW},   {"no", FORWARD_DENY},
    {"remote", FORWARD_REMOTE}, {"local", FORWARD_LOCAL}, {NULL, -1}};

int process_server_config_line(ServerOptions *options, char *line,
  ...
  case sListenAddress:
    ...
    arg2 = NULL;
    if ((arg = strdelim(&cp)) != NULL) {
      if (strcmp(arg, "rdomain") != 0 || (arg2 = strdelim(&cp)) == NULL)
        fatal("%s line %d: bad ListenAddress syntax", filename, linenum);
      if (!valid_rdomain(arg2))
        fatal("%s line %d: bad routing domain", filename, linenum);
    }
  ...
  case sHostKeyAgent:
    ...
    arg = strdelim(&cp);
    if (!arg || *arg == '\0')
      fatal("%s line %d: missing socket name.", filename, linenum);
    if (*activep && *charptr == NULL)
      *charptr = !strcmp(arg, SSH_AUTHSOCKET_ENV_NAME) ? xstrdup(arg)
  ...
  parse_keytypes:
    ...
    if (!arg || *arg == '\0')
      fatal("%s line %d: Missing argument.", filename, linenum);
    if (*arg != '-' && !sshkey_names_valid2(*arg == '+' ? arg + 1 : arg, 1))
      fatal("%s line %d: Bad key types '%s'.", filename, linenum,
            arg ? arg : "<NONE>");
    if (*activep && *charptr == NULL)
      *charptr = xstrdup(arg);
    break;
  ...
  parse_int:
    ...
    if ((errstr = atoi_err(arg, &value)) != NULL)
      fatal("%s line %d: integer value %s.", filename, linenum, errstr);
    if (*activep && *intptr == -1)
      *intptr = value;
    break;
  ...
  case sRekeyLimit:
    ...
    if (strcmp(arg, "default") == 0) {
      ...
      if (scan_scaled(arg, &val64) == -1)
        fatal("%.200s line %d: Bad number '%s': %s", filename, linenum, arg,
              strerror(errno));
      if (val64 != 0 && val64 < 16)
  ...
  case sLogLevel:
    ...
    if (value == SYSLOG_LEVEL_NOT_SET)
      fatal("%.200s line %d: unsupported log level '%s'", filename, linenum,
            arg ? arg : "<NONE>");
  ...
  case sAllowUsers:
    while ((arg = strdelim(&cp)) && *arg != '\0') {
      ...
      if (!*activep)
        continue;
      array_append(filename, linenum, "AllowUsers", &options->allow_users,
                   &options->num_allow_users, arg);
    }
    break;
  ...
  case sDenyUsers:
    while ((arg = strdelim(&cp)) && *arg != '\0') {
      ...
      if (!*activep)
        continue;
      array_append(filename, linenum, "DenyUsers", &options->deny_users,
                   &options->num_deny_users, arg);
    }
    break;
  ...
  case sAllowGroups:
    while ((arg = strdelim(&cp)) && *arg != '\0') {
      if (!*activep)
        continue;
      array_append(filename, linenum, "AllowGroups", &options->allow_groups,
                   &options->num_allow_groups, arg);
    }
    break;
  ...
  case sDenyGroups:
    while ((arg = strdelim(&cp)) && *arg != '\0') {
      if (!*activep)
        continue;
      array_append(filename, linenum, "DenyGroups", &options->deny_groups,
                   &options->num_deny_groups, arg);
    }
    break;
  ...
  case sCiphers:
    ...
    if (*arg != '-' && !ciphers_valid(*arg == '+' ? arg + 1 : arg))
      fatal("%s line %d: Bad SSH2 cipher spec '%s'.", filename, linenum,
            arg ? arg : "<NONE>");
  ...
  case sMacs:
    ...
    if (*arg != '-' && !mac_valid(*arg == '+' ? arg + 1 : arg))
      fatal("%s line %d: Bad SSH2 mac spec '%s'.", filename, linenum,
            arg ? arg : "<NONE>");
  ...
  case sKexAlgorithms:
    ...
    if (!arg || *arg == '\0')
      fatal("%s line %d: Missing argument.", filename, linenum);
    if (*arg != '-' && !kex_names_valid(*arg == '+' ? arg + 1 : arg))
      fatal("%s line %d: Bad SSH2 KexAlgorithms '%s'.", filename, linenum,
            arg ? arg : "<NONE>");
  ...
  case sMaxStartups:
    ...
    if ((n = sscanf(arg, "%d:%d:%d", &options->max_startups_begin,
                    &options->max_startups_rate, &options->max_startups)) ==
        3) {
      if (options->max_startups_begin > options->max_startups ||
          options->max_startups_rate > 100 || options->max_startups_rate < 1)
  ...
  case sAuthorizedKeysFile:
    if (*activep && options->num_authkeys_files == 0) {
      while ((arg = strdelim(&cp)) && *arg != '\0') {
        ...
        array_append(filename, linenum, "AuthorizedKeysFile",
  ...
  case sAcceptEnv:
    while ((arg = strdelim(&cp)) && *arg != '\0') {
      ...
      if (!*activep)
        continue;
      array_append(filename, linenum, "AcceptEnv", &options->accept_env,
                   &options->num_accept_env, arg);
    }
    break;
  ...
  case sSetEnv:
    ...
    while ((arg = strdelimw(&cp)) && *arg != '\0') {
      if (strchr(arg, '=') == NULL)
        fatal("%s line %d: Invalid environment.", filename, linenum);
      if (!*activep || uvalue != 0)
        continue;
      array_append(filename, linenum, "SetEnv", &options->setenv,
                   &options->num_setenv, arg);
    }
    break;
  ...
  case sPermitOpen:
    ...
    arg = strdelim(&cp);
    if (!arg || *arg == '\0')
      fatal("%s line %d: missing %s specification", filename, linenum,
            lookup_opcode_name(opcode));
    uvalue = *uintptr; /* modified later */
    if (strcmp(arg, "any") == 0 || strcmp(arg, "none") == 0) {
      if (*activep && uvalue == 0) {
        *uintptr = 1;
        *chararrayptr = xcalloc(1, sizeof(**chararrayptr));
        (*chararrayptr)[0] = xstrdup(arg);
      }
      break;
    }
    for (; arg != NULL && *arg != '\0'; arg = strdelim(&cp)) {
      if (opcode == sPermitListen && strchr(arg, ':') == NULL) {
        /*
        ...
        p = hpdelim(&arg);
        if (p == NULL) {
          fatal("%s line %d: missing host in %s", filename, linenum,
                lookup_opcode_name(opcode));
        }
        p = cleanhostname(p);
      }
      if (arg == NULL || ((port = permitopen_port(arg)) < 0)) {
        fatal("%s line %d: bad port number in %s", filename, linenum,
              lookup_opcode_name(opcode));
      }
      if (*activep && uvalue == 0) {
        array_append(filename, linenum, lookup_opcode_name(opcode),
  ...
  case sAuthorizedPrincipalsCommand:
    if (cp == NULL)
      fatal("%.200s line %d: Missing argument.", filename, linenum);
    len = strspn(cp, WHITESPACE);
    if (*activep && options->authorized_principals_command == NULL) {
      if (cp[len] != '/' && strcasecmp(cp + len, "none") != 0)
        fatal("%.200s line %d: "
  ...
  case sAuthenticationMethods:
    if (options->num_auth_methods == 0) {
      ...
      while ((arg = strdelim(&cp)) && *arg != '\0') {
        ...
        if (!*activep)
          continue;
        array_append(filename, linenum, "AuthenticationMethods",
                     &options->auth_methods, &options->num_auth_methods, arg);
      }
      if (value2 == 0) {
  ...
  case sFingerprintHash:
    ...
    if ((value = ssh_digest_alg_by_name(arg)) == -1)
      fatal("%.200s line %d: Invalid hash algorithm \"%s\".", filename, linenum,
            arg);
    if (*activep)
  ...
  case sIgnore:
  case sUnsupported:
    do_log2(opcode == sIgnore ? SYSLOG_LEVEL_DEBUG2 : SYSLOG_LEVEL_INFO,
  ...
  default:
    fatal("%s line %d: Missing handler for opcode %s (%d)", filename, linenum,
          arg, opcode);
  }
  if ((arg = strdelim(&cp)) != NULL && *arg != '\0')
...
void load_server_config(const char *filename, struct sshbuf *conf) {
  char *line = NULL, *cp;
  size_t linesize = 0;
  FILE *f;
  int r, lineno = 0;
...
#define M_CP_STRARRAYOPT(s, num_s)                                             \
  do {                                                                         \
    u_int i;                                                                   \
    if (src->num_s != 0) {                                                     \
...
void parse_server_config(ServerOptions *options, const char *filename,
                         struct sshbuf *conf,
                         struct connection_info *connectinfo) {
  ...
  while ((cp = strsep(&cbuf, "\n")) != NULL) {
    if (process_server_config_line(options, cp, filename, linenum++, &active,
                                   connectinfo) != 0)
      bad_options++;
  }
  free(obuf);
  if (bad_options > 0)
    fatal("%s: terminating, %d bad configuration options", filename,
          bad_options);
...
static const char *fmt_intarg(ServerOpCodes code, int val) {
  if (val == -1)
    return "unset";
  switch (code) {
  case sAddressFamily:
...
static void dump_cfg_oct(ServerOpCodes code, int val) {
  printf("%s 0%o\n", lookup_opcode_name(code), val);
}
...
static void dump_cfg_strarray_oneline(ServerOpCodes code, u_int count,
                                      char **vals) {
  u_int i;
...
static char *format_listen_addrs(struct listenaddr *la) {
  int r;
  struct addrinfo *ai;
  ...
  for (ai = la->addrs; ai; ai = ai->ai_next) {
    if ((r = getnameinfo(ai->ai_addr, ai->ai_addrlen, addr, sizeof(addr), port,
    ...
    if (ai->ai_family == AF_INET6) {
      xasprintf(&laddr1, "listenaddress [%s]:%s%s%s\n%s", addr, port,
                ...
                la->rdomain == NULL ? "" : la->rdomain, laddr2);
    } else {
      xasprintf(&laddr1, "listenaddress %s:%s%s%s\n%s", addr, port,
                la->rdomain == NULL ? "" : " rdomain ",
                la->rdomain == NULL ? "" : la->rdomain, laddr2);
...
void dump_config(ServerOptions *o) {
  ...
  dump_cfg_string(sAuthorizedPrincipalsFile, o->authorized_principals_file);
  dump_cfg_string(sVersionAddendum,
                  *o->version_addendum == '\0' ? "none" : o->version_addendum);
  dump_cfg_string(sAuthorizedKeysCommand, o->authorized_keys_command);
  dump_cfg_string(sAuthorizedKeysCommandUser, o->authorized_keys_command_user);
  dump_cfg_string(sAuthorizedPrincipalsCommand,
                  o->authorized_principals_command);
  dump_cfg_string(sAuthorizedPrincipalsCommandUser,
                  o->authorized_principals_command_user);
  dump_cfg_string(sHostKeyAgent, o->host_key_agent);
  dump_cfg_string(sKexAlgorithms,
                  o->kex_algorithms ? o->kex_algorithms : KEX_SERVER_KEX);
  dump_cfg_string(sCASignatureAlgorithms, o->ca_sign_algorithms
                                              ? o->ca_sign_algorithms
                                              : SSH_ALLOWED_CA_SIGALGS);
  dump_cfg_string(sHostbasedAcceptedKeyTypes, o->hostbased_key_types
                                                  ? o->hostbased_key_types
                                                  : KEX_DEFAULT_PK_ALG);
  ...
  dump_cfg_strarray(sHostKeyFile, o->num_host_key_files, o->host_key_files);
  ...
  dump_cfg_strarray_oneline(sAuthenticationMethods, o->num_auth_methods,
                            o->auth_methods);
  ...
  printf("maxstartups %d:%d:%d\n", o->max_startups_begin, o->max_startups_rate,
         o->max_startups);
