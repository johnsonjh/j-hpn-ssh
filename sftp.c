...
#define VIEW_FLAGS                                                             \
  (LS_LONG_VIEW | LS_SHORT_VIEW | LS_NUMERIC_VIEW | LS_SI_UNITS)
...
static const struct CMD cmds[] = {{"bye", I_QUIT, NOARGS},
                                  {"cd", I_CHDIR, REMOTE},
...
/* ARGSUSED */
static void suspchild(int signo) {
  if (sshpid > 1) {
    kill(sshpid, signo);
...
static void help(void) {
  printf(
      ...
      "chgrp [-h] grp path                Change group of file 'path' to "
      "'grp'\n"
      "chmod [-h] mode path               Change permissions of file 'path' to "
      "'mode'\n"
      "chown [-h] own path                Change owner of file 'path' to "
      "'own'\n"
...
static void local_do_shell(const char *args) {
  int status;
  char *shell;
  pid_t pid;
...
static void local_do_ls(const char *args) {
  if (!args || !*args)
...
static char *path_strip(const char *path, const char *strip) {
  size_t len;
...
static char *make_absolute(char *p, const char *pwd) {
  char *abs_str;
...
static int parse_rename_flags(const char *cmd, char **argv, int argc,
                              int *lflag) {
  extern int opterr, optind, optopt, optreset;
  int ch;
...
static int parse_ls_flags(char **argv, int argc, int *lflag) {
  extern int opterr, optind, optopt, optreset;
  int ch;
...
static int parse_no_flags(const char *cmd, char **argv, int argc) {
  extern int opterr, optind, optopt, optreset;
  int ch;
...
static int remote_is_dir(struct sftp_conn *conn, const char *path) {
  Attrib *a;
...
static int pathname_is_dir(const char *pathname) {
  size_t l = strlen(pathname);

  return l > 0 && pathname[l - 1] == '/';
}

static int process_get(struct sftp_conn *conn, const char *src, const char *dst,
                       const char *pwd, int pflag, int rflag, int resume,
                       int fflag) {
  char *abs_src = NULL;
  char *abs_dst = NULL;
  glob_t g;
  ...
  for (i = 0; g.gl_pathv[i] && !interrupted; i++) {
    ...
    if (pathname_is_dir(g.gl_pathv[i]) && (rflag || global_rflag)) {
      ...
      if (do_download(conn, g.gl_pathv[i], abs_dst, NULL, pflag || global_pflag,
                      resume, fflag || global_fflag) == -1)
        err = -1;
    }
    free(abs_dst);
    abs_dst = NULL;
...
static int process_put(struct sftp_conn *conn, const char *src, const char *dst,
                       const char *pwd, int pflag, int rflag, int resume,
                       int fflag) {
  char *tmp_dst = NULL;
  char *abs_dst = NULL;
  ...
  for (i = 0; g.gl_pathv[i] && !interrupted; i++) {
    ...
    if (pathname_is_dir(g.gl_pathv[i]) && (rflag || global_rflag)) {
      if (upload_dir(conn, g.gl_pathv[i], abs_dst, pflag || global_pflag, 1,
                     resume, fflag || global_fflag) == -1)
        err = -1;
    } else {
      if (do_upload(conn, g.gl_pathv[i], abs_dst, pflag || global_pflag, resume,
                    fflag || global_fflag) == -1)
        err = -1;
    }
  }
...
static int sdirent_comp(const void *aa, const void *bb) {
...
static int do_ls_dir(struct sftp_conn *conn, const char *path,
...
static int sglob_comp(const void *aa, const void *bb) {
...
static int do_globbed_ls(struct sftp_conn *conn, const char *path,
                         const char *strip_path, int lflag) {
  char *fname, *lname;
  glob_t g;
  int err, r;
  ...
  if ((r = remote_glob(conn, path,
                       GLOB_MARK | GLOB_NOCHECK | GLOB_BRACE | GLOB_KEEPSTAT |
                           GLOB_NOSORT,
                       NULL, &g)) != 0 ||
  ...
  for (j = 0; j < nentries && !interrupted; j++) {
    ...
    if (lflag & LS_LONG_VIEW) {
      ...
      lname = ls_file(fname, g.gl_statv[i], 1, (lflag & LS_SI_UNITS));
      mprintf("%s\n", lname);
      free(lname);
    } else {
      mprintf("%-*s", colspace, fname);
...
static int do_df(struct sftp_conn *conn, const char *path, int hflag,
                 int iflag) {
  ...
  if (st.f_files == 0)
    ...
    snprintf(
        s_icapacity, sizeof(s_icapacity), "%3llu%%",
        (unsigned long long)(100 * (st.f_files - st.f_ffree) / st.f_files));
  }
  if (st.f_blocks == 0)
    strlcpy(s_dcapacity, "ERR", sizeof(s_dcapacity));
  else {
    snprintf(
        s_dcapacity, sizeof(s_dcapacity), "%3llu%%",
        (unsigned long long)(100 * (st.f_blocks - st.f_bfree) / st.f_blocks));
  }
  if (iflag) {
    ...
    printf("%11llu %11llu %11llu %11llu         %s\n",
           ...
           (unsigned long long)st.f_favail, (unsigned long long)st.f_ffree,
           s_icapacity);
  } else if (hflag) {
    ...
    printf(
        ...
        (unsigned long long)(st.f_frsize * (st.f_blocks - st.f_bfree) / 1024),
        (unsigned long long)(st.f_frsize * st.f_bavail / 1024),
        (unsigned long long)(st.f_frsize * st.f_bfree / 1024), s_dcapacity);
  }
  return 0;
}
 ...
 * does not glob it.
 */
static void undo_glob_escape(char *s) {
  size_t i, j;
...
static char **makeargv(const char *arg, int *argcp, int sloppy, char *lastquote,
                       u_int *terminated) {
  int argc, quot;
  size_t i, j;
...
static int parse_args(const char **cpp, int *ignore_errors, int *disable_echo,
                      int *aflag, int *fflag, int *hflag, int *iflag,
  ...
  *path1 = *path2 = NULL;
  ...
  case I_REPUT:
  case I_PUT:
    if ((optidx = parse_getput_flags(cmd, argv, argc, aflag, fflag, pflag,
                                     rflag)) == -1)
      return -1;
  ...
  case I_LCHDIR:
    ...
    if (argc - optidx < 1) {
      ...
      error("You must specify a path after a %s command.", cmd);
      return -1;
    }
    *path1 = xstrdup(argv[optidx]);
    /* Only "rm" globs */
    ...
    break;
  case I_DF:
    if ((optidx = parse_df_flags(cmd, argv, argc, hflag, iflag)) == -1)
      return -1;
  ...
  case I_CHGRP:
    ...
    if (cp2 == argv[optidx] || *cp2 != '\0' ||
        ((l == LONG_MIN || l == LONG_MAX) && errno == ERANGE) || l < 0) {
    need_num_arg:
...
static int parse_dispatch_command(struct sftp_conn *conn, const char *cmd,
                                  char **pwd, const char *startdir,
                                  int err_abort, int echo_command) {
  const char *ocmd = cmd;
  ...
  case I_CHMOD:
    ...
    for (i = 0; g.gl_pathv[i] && !interrupted; i++) {
      ...
      err = (hflag ? do_lsetstat : do_setstat)(conn, g.gl_pathv[i], &a);
      if (err != 0 && err_abort)
        break;
    }
    break;
  ...
  case I_CHGRP:
    ...
    remote_glob(conn, path1, GLOB_NOCHECK, NULL, &g);
    for (i = 0; g.gl_pathv[i] && !interrupted; i++) {
      if (!(aa = (hflag ? do_lstat : do_stat)(conn, g.gl_pathv[i], 0))) {
        if (err_abort) {
          err = -1;
          break;
        } else
      ...
      err = (hflag ? do_lsetstat : do_setstat)(conn, g.gl_pathv[i], aa);
      if (err != 0 && err_abort)
        break;
    }
    break;
...
#ifdef USE_LIBEDIT
static char *prompt(EditLine *el) { return ("sftp> "); }

/* Display entries in 'list' after skipping the first 'len' chars */
static void complete_display(char **list, u_int len) {
 ...
 */
static char *complete_ambiguous(const char *word, char **list, size_t count) {
  if (word == NULL)
    return NULL;
...
static int complete_cmd_parse(EditLine *el, char *cmd, int lastarg, char quote,
                              int terminated) {
 ...
 */
static int complete_is_remote(char *cmd) {
  int i;
...
static int complete_match(EditLine *el, struct sftp_conn *conn,
                          char *remote_path, char *file, int remote,
                          int lastarg, char quote, int terminated) {
  glob_t g;
  char *tmp, *tmp2, ins[8];
  ...
  if (g.gl_matchc == 1) {
    ...
    if (!terminated && quote != '\0')
      ins[i++] = quote;
    if (*(lf->cursor - 1) != '/' && (lastarg || *(lf->cursor) != ' '))
      ins[i++] = ' ';
    ins[i] = '\0';
...
static unsigned char complete(EditLine *el, int ch) {
  ...
  if (line[cursor] != ' ' && line[cursor] != '\0' && line[cursor] != '\n') {
    free(line);
    return ret;
  }
  ...
  if (carg == 0) {
    ...
    if (complete_cmd_parse(el, argv[0], argc == carg, quote, terminated) != 0)
      ret = CC_REDISPLAY;
  } else if (carg >= 1) {
    /* Handle file parsing */
    ...
    if (remote != 0 &&
        complete_match(el, complete_ctx->conn, *complete_ctx->remote_pathp,
                       filematch, remote, carg == argc, quote, terminated) != 0)
      ret = CC_REDISPLAY;
  }
...
static int interactive_loop(struct sftp_conn *conn, char *file1, char *file2) {
  char *remote_path;
  ...
  if (file1 != NULL) {
    ...
    if (remote_is_dir(conn, dir) && file2 == NULL) {
      ...
      if (parse_dispatch_command(conn, cmd, &remote_path, startdir, 1, 0) !=
          0) {
        free(dir);
        free(startdir);
        free(remote_path);
      ...
      snprintf(cmd, sizeof cmd, "get%s %s%s%s", global_aflag ? " -a" : "", dir,
               file2 == NULL ? "" : " ", file2 == NULL ? "" : file2);
  ...
  for (;;) {
    ...
    err = parse_dispatch_command(conn, cmd, &remote_path, startdir, batchmode,
                                 !interactive && el == NULL);
    if (err != 0)
      break;
  }
...
int main(int argc, char **argv) {
  ...
  while ((ch = getopt(argc, argv,
    ...
    case 'b':
      ...
      /* Allow "-" as stdin */
      if (strcmp(optarg, "-") != 0 && (infile = fopen(optarg, "r")) == NULL)
    ...
    case 'D':
      ...
      break;
    case 'l':
      limit_kbps = strtonum(optarg, 1, 100 * 1024 * 1024, &errstr);
      if (errstr != NULL)
        usage();
    ...
    case 'R':
      ...
      if (num_requests == 0 || *cp != '\0')
        fatal("Invalid number of requests \"%s\"", optarg);
      break;
    case 's':
      sftp_server = optarg;
      break;
  ...
  if (sftp_direct == NULL) {
    ...
    case 0:
      ...
      break;
    default:
      if (parse_user_host_path(*argv, &user, &host, &file1) == -1) {
